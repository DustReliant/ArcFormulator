...
void QdfDockAreaTitleBarPrivate::createButtons()
{
    ...
    internal::setButtonIcon(tabsMenuButton, QStyle::SP_TitleBarUnshadeButton,
                            DockWidgetTabIcon::DockAreaMenu);
    ...
    internal::setButtonIcon(undockButton, QStyle::SP_TitleBarNormalButton,
                            DockWidgetTabIcon::DockAreaUndock);
    ...
    autoHideButton = new QdfTitleBarButton(
            testAutoHideConfigFlag(QdfDockManager::DockAreaHasAutoHideButton) && autoHideEnabled);
    ...
    internal::setButtonIcon(closeButton, QStyle::SP_TitleBarCloseButton,
                            DockWidgetTabIcon::DockAreaClose);
...
QdfFloatingWidget *QdfDockAreaTitleBarPrivate::makeAreaFloating(const QPoint &offset, DragState state)
{
    ...
    else
    {
        ...
        QObject::connect(w, &QdfFloatingDragPreview::draggingCanceled, [=]() {
            this->dragState = DragState::DS_Inactive;
        });
        pFloatingWidget = w;
    }
...
QAction *QdfDockAreaTitleBarPrivate::createAutoHideToAction(const QString &title,
...
QdfDockAreaTitleBar::QdfDockAreaTitleBar(QdfDockAreaWidget *parent) : QFrame(parent)
{
...
{
    QDF_D(QdfDockAreaTitleBar)
    if (QdfDockAreaTitleBarPrivate::testConfigFlag(
                QdfDockManager::DockAreaDynamicTabsMenuButtonVisibility))
    {
        ...
        QMetaObject::invokeMethod(d->tabsMenuButton, "setVisible", Qt::QueuedConnection,
                                  Q_ARG(bool, visible));
    }
    d->menuOutdated = true;
}
...
void QdfDockAreaTitleBar::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (d->dockArea->dockContainer()->isFloating() &&
        d->dockArea->dockContainer()->visibleDockAreaCount() == 1 && !d->dockArea->autoHide())
    {
        return;
    }
    auto Features = d->dockArea->features();
    if (!Features.testFlag(QdfDockWidget::DockWidgetFloatable) &&
        !(Features.testFlag(QdfDockWidget::DockWidgetMovable)))
    {
        return;
    }
...
void QdfDockAreaTitleBar::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (!isTopLevelArea)
    {
        action = pMenu.addAction(autoHide ? tr("Detach") : tr("Detach Group"), this,
        ...
        {
            action = pMenu.addAction(autoHide ? tr("Unpin (Dock)") : tr("Pin Group"), this,
    ...
    action = pMenu.addAction(autoHide ? tr("Close") : tr("Close Group"), this,
...
QdfTitleBarButton::QdfTitleBarButton(bool visible, QWidget *parent)
    : QToolButton(parent), Visible(visible),
...
QdfSpacerWidget::QdfSpacerWidget(QWidget *parent) : QWidget(parent)
{
