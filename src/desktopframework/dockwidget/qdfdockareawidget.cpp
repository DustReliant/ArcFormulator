...
QdfDockAreaLayout::QdfDockAreaLayout(QBoxLayout *parent) : m_parentLayout(parent)
{
}
...
void QdfDockAreaWidgetPrivate::createTitleBar()
{
    ...
    QObject::connect(tabBar(), &QdfDockAreaTabBar::tabCloseRequested, q,
                     &QdfDockAreaWidget::onTabCloseRequested);
    QObject::connect(titleBar, &QdfDockAreaTitleBar::tabBarClicked, q,
                     &QdfDockAreaWidget::setCurrentIndex);
...
void QdfDockAreaWidgetPrivate::updateTitleBarButtonStates()
{
    ...
    titleBar->button(TitleBarButton::Close)
            ->setEnabled(q->features().testFlag(QdfDockWidget::DockWidgetClosable));
    titleBar->button(TitleBarButton::Undock)
            ->setEnabled(q->features().testFlag(QdfDockWidget::DockWidgetFloatable));
    titleBar->button(TitleBarButton::AutoHide)
            ->setEnabled(q->features().testFlag(QdfDockWidget::DockWidgetPinnable));
...
QdfDockAreaWidget::~QdfDockAreaWidget(){QDF_FINI_PRIVATE()}
...
void QdfDockAreaWidget::insertDockWidget(int index, QdfDockWidget *dockWidget, bool Activate)
{
    QDF_D(QdfDockAreaWidget)
...
void QdfDockAreaWidget::removeDockWidget(QdfDockWidget *dockWidget)
{
    ...
    auto NextOpenDockWidget =
            (dockWidget == CurrentDockWidget) ? nextOpenDockWidget(dockWidget) : nullptr;
...
void QdfDockAreaWidget::hideAreaWithNoVisibleContent()
{
    ...
    if (!Container->isFloating() &&
        !QdfDockManager::testConfigFlag(QdfDockManager::HideSingleCentralWidgetTitleBar))
    {
        return;
    }
...
void QdfDockAreaWidget::onTabCloseRequested(int index)
{
    ...
    if (pDockWidget->features().testFlag(QdfDockWidget::DockWidgetDeleteOnClose) ||
        pDockWidget->features().testFlag(QdfDockWidget::CustomCloseHandling))
    {
...
void QdfDockAreaWidget::closeArea()
{
    auto OpenDockWidgets = openedDockWidgets();
    if (OpenDockWidgets.count() == 1 &&
        (OpenDockWidgets[0]->features().testFlag(QdfDockWidget::DockWidgetDeleteOnClose) ||
         OpenDockWidgets[0]->features().testFlag(QdfDockWidget::CustomCloseHandling)) &&
        !autoHide())
    {
    ...
    else
    {
        ...
        {
            if ((dockWidget->features().testFlag(QdfDockWidget::DockWidgetDeleteOnClose) &&
                 dockWidget->features().testFlag(QdfDockWidget::DockWidgetForceCloseWithArea)) ||
                dockWidget->features().testFlag(QdfDockWidget::CustomCloseHandling))
            {
                dockWidget->closeDockWidgetInternal();
            }
            else if (dockWidget->features().testFlag(QdfDockWidget::DockWidgetDeleteOnClose) &&
                     autoHide())
            {
                dockWidget->closeDockWidgetInternal();
            }
