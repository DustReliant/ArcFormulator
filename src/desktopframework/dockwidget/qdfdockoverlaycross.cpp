...
QPixmap QdfDockOverlayCrossPrivate::createHighDpiDropIndicatorPixmap(const QSizeF &size,
                                                                     DockWidgetArea area,
                                                                     QdfDockOverlay::Mode mode)
{
    QDF_Q(QdfDockOverlayCross)
...
QdfDockOverlayCross::QdfDockOverlayCross(QdfDockOverlay *overlay) : QWidget(overlay->parentWidget())
{
...
DockWidgetArea QdfDockOverlayCross::cursorLocation() const
{
    ...
    {
        iter.next();
        if (d->dockOverlay->allowedAreas().testFlag(iter.key()) && iter.value() &&
            iter.value()->isVisible() && iter.value()->geometry().contains(pos))
        {
            return iter.key();
        }
    }
...
void QdfDockOverlayCross::reset()
{
    ...
    allAreas << TopDockWidgetArea << RightDockWidgetArea << BottomDockWidgetArea << LeftDockWidgetArea
