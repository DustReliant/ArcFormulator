...
void QdfDockWidgetPrivate::showDockWidget()
{
    ...
    if (!widget)
    {
        ...
        {
            Q_ASSERT(!features.testFlag(QdfDockWidget::DeleteContentOnClose) &&
    ...
    else
    {
        ...
        {
            QdfFloatingDockContainer *floatingWidget =
                    internal::findParent<QdfFloatingDockContainer *>(container);
            floatingWidget->show();
        }

        if (container->openedDockWidgets().count() == 0 && dockArea->autoHide() &&
...
QdfDockWidget::QdfDockWidget(const QString &title, QWidget *parent) : QFrame(parent)
{
...
void QdfDockWidget::toggleViewInternal(bool open)
{
    ...
    QdfDockWidget *topLevelDockWidgetBefore =
    ...
    QdfDockWidget *TopLevelDockWidgetAfter =
            DockContainer ? DockContainer->topLevelDockWidget() : nullptr;
    QdfDockWidget::emitTopLevelEventForWidget(TopLevelDockWidgetAfter, true);
    QdfFloatingDockContainer *FloatingContainer =
...
bool QdfDockWidget::closeDockWidgetInternal(bool forceClose)
{
    ...
    if (features().testFlag(QdfDockWidget::DockWidgetDeleteOnClose))
    {
        ...
        {
            QdfFloatingDockContainer *FloatingWidget =
                    internal::findParent<QdfFloatingDockContainer *>(this);
...
void QdfDockWidget::setAutoHide(bool enable, SideBarLocation location)
{
    ...
    {
        auto area =
                (SideBarLocation::SL_None == location) ? dockArea->calculateSideTabBarArea() : location;
