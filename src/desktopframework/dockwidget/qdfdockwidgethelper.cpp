...
void xcb_add_prop(bool state, WId window, const char *type, const char *prop)
{
    ...
    xcb_connection_t *connection = x11_connection();
    xcb_change_property(connection, XCB_PROP_MODE_REPLACE, window, type_atom, XCB_ATOM_ATOM, 32,
...
void setButtonIcon(QAbstractButton *button, QStyle::StandardPixmap StandarPixmap, DockWidgetTabIcon icon)
{
...
void repolishStyle(QWidget *w, RepolishChildOptions options)
{
    ...
    QList<QWidget *> Children = w->findChildren<QWidget *>(
