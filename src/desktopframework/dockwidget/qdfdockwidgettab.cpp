...
void QdfDockWidgetTabPrivate::updateCloseButtonSizePolicy()
{
    ...
    sizePolicy.setRetainSizeWhenHidden(
            features.testFlag(QdfDockWidget::DockWidgetClosable) &&
            testConfigFlag(QdfDockManager::RetainTabSizeWhenCloseButtonHidden));
...
QAction *QdfDockWidgetTabPrivate::createAutoHideToAction(const QString &title, SideBarLocation location,
                                                         QMenu *menu)
{
    QDF_Q(QdfDockWidgetTab)
...
QdfDockWidgetTab::QdfDockWidgetTab(QdfDockWidget *dockWidget, QWidget *parent) : QFrame(parent)
{
...
void QdfDockWidgetTab::mouseMoveEvent(QMouseEvent *event)
{
    ...
    int DragDistanceY =
            qAbs(d->globalDragStartMousePosition.y() - internal::globalPositionOf(event).y());
    if (DragDistanceY >= QdfDockManager::startDragDistance() || MouseOutsideBar)
    {
        if (d->dockArea->dockContainer()->isFloating() && d->dockArea->openedDockWidgetsCount() == 1 &&
    ...
    else if (d->dockArea->openedDockWidgetsCount() > 1 &&
             (internal::globalPositionOf(event) - d->globalDragStartMousePosition).manhattanLength() >=
...
void QdfDockWidgetTab::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton)
    {
        if ((!d->dockArea->dockContainer()->isFloating() || d->dockArea->dockWidgetsCount() > 1) &&
            d->dockWidget->features().testFlag(QdfDockWidget::DockWidgetFloatable))
        {
            event->accept();
            d->saveDragStartMousePosition(internal::globalPositionOf(event));
