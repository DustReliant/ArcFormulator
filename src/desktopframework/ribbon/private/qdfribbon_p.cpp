...
QdfRibbonGroupPopup *QdfRibbonGroupPopupButton::popupWidget()
{
...
bool QdfRibbonGroupPopupButton::isShowPopup() const
{
    return m_hasPopup;
}

void QdfRibbonGroupPopupButton::setVisible(bool visible)
{
...
QSize QdfRibbonGroupPopupButton::sizeHint() const
{
    ...
    initStyleOption(&opt);
    return QSize(style()->pixelMetric((QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonReducedGroupWidth,
                                      &opt, this),
...
void QdfRibbonGroupPopup::paintEvent(QPaintEvent *event)
{
    ...
    opt.text = p.fontMetrics().elidedText(opt.text, m_group->titleElideMode(),
                                          rcEntryCaption.adjusted(2, 0, -2, 0).width());
    ribbonStyle->drawItemText(&p, rcEntryCaption, flags, opt.palette, opt.state & QStyle::State_Enabled,
...
QString QdfRibbonGroupOption::text() const
{
    return "";
}

QSize QdfRibbonGroupOption::sizeHint() const
{
    return QSize(15, 14);
}
...
void QdfRibbonGroupOption::paintEvent(QPaintEvent *event)
{
    ...
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonOptionButton, &opt, &p,
                           this);
}
...
QdfRibbonGroupScroll::QdfRibbonGroupScroll(QWidget *parent, bool scrollLeft)
    : QToolButton(parent), m_scrollLeft(scrollLeft)
{
}
...
void QdfRibbonGroupScroll::paintEvent(QPaintEvent *event)
{
    ...
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonGroupScrollButton, &opt,
                           &p, this);
}
...
QdfKeyTipEvent::QdfKeyTipEvent(QdfRibbonKeyTip *kTip) : QEvent(QdfKeyTipEvent::eventNumber())
{
    keyTip = kTip;
}

QdfRibbonKeyTip *QdfKeyTipEvent::getKeyTip() const
{
    return keyTip;
}
...
QdfShowKeyTipEvent::QdfShowKeyTipEvent(QWidget *w) : QEvent(QdfShowKeyTipEvent::eventNumber())
{
    m_buddy = w;
}

QWidget *QdfShowKeyTipEvent::buddy() const
{
    return m_buddy;
}
