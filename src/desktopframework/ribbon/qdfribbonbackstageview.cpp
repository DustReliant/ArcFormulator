...
void QdfRibbonBackstageCloseButton::paintEvent(QPaintEvent *event)
{
    ...
    initStyleOption(&opt);
    QPainter p(this);
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonBackstageCloseButton,
                           &opt, &p, this);
}
...
QdfRibbonBackstageViewMenu::QdfRibbonBackstageViewMenu(QdfRibbonBackstageView *backstageView,
                                                       QWidget *parent)
    : QWidget(parent), m_backstageView(backstageView), m_backStageCloseButton(nullptr),
      m_mouseDown(false), m_itemsDirty(true), m_menuWidth(132), m_maxIconWidth(0)
{
...
void QdfRibbonBackstageViewMenu::resetItemsDirty()
{
    m_itemsDirty = true;
}
...
void QdfRibbonBackstageViewMenu::initStyleOption(QStyleOptionMenuItem *option,
                                                 const QAction *action) const
{
    ...
    if (m_backstageView->qdf_d()->m_currentAction &&
        m_backstageView->qdf_d()->m_currentAction == action &&
...
void QdfRibbonBackstageViewMenu::updateActionRects() const
{
    ...
    for (int i = 0, count = actions.count(); i < count; i++)
    {
        ...
        {
            m_actionRects[i] = QRect(QPoint(-commandGap, top),
                                     QPoint(m_menuWidth - commandGap * 2, top + actionHeight - 3));
            top += actionHeight - 3;
        }
        else
        {
            m_actionRects[i] =
                    QRect(QPoint(commandGap, top), QPoint(m_menuWidth - commandGap, top + actionHeight));
            top += actionHeight + 2;
        }
    }
...
void QdfRibbonBackstageViewMenu::paintEvent(QPaintEvent *event)
{
    ...
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_Backstage, &optBackstage, &p,
                           m_backstageView);
...
void QdfRibbonBackstageViewMenu::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (rect().contains(event->pos()))
    {
        ...
        if (action && (action->isSeparator() || !action->isEnabled() ||
                       m_backstageView->qdf_d()->m_widgetItems.contains(action)))
        {
            return;
        }
...
QdfRibbonBackstageButton::~QdfRibbonBackstageButton()
{
    QDF_FINI_PRIVATE();
}
...
QdfRibbonBackstagePage::QdfRibbonBackstagePage(QWidget *parent) : QWidget(parent)
{
...
void QdfRibbonBackstageViewPrivate::layoutBackstage()
{
    ...
    m_backstageMenu->setGeometry(
            QRect(QPoint(rect.topLeft()), QPoint(rect.left() + sz.width(), rect.bottom())));
...
QAction *QdfRibbonBackstageViewPrivate::currentAction() const
{
...
QdfRibbonBackstageView::QdfRibbonBackstageView(QdfRibbonBar *ribbonBar)
...
QdfRibbonBackstageView::~QdfRibbonBackstageView()
{
    QDF_FINI_PRIVATE();
}
