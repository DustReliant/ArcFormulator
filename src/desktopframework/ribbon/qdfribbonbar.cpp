...
void QdfRibbonBarPrivate::init()
{
    ...
    m_tabBar->show();
    QObject::connect(m_tabBar, SIGNAL(currentChanged(int)), this, SLOT(currentChanged(int)),
                     Qt::DirectConnection);
...
void QdfRibbonBarPrivate::calcHeightItem()
{
    QDF_Q(QdfRibbonBar);
    {
        ...
        int hframe =
                qMax(tab.sizeHint().height(), (isDPIAware ? int(QdfDrawHelpers::dpiScaled(24.)) : 24));
...
void QdfRibbonBarPrivate::layoutRibbon()
{
    ...
    QRect rcPageArea(QPoint(0, tabsHeight),
                     QSize(opt.rect.width(), pageAreaHeight + vmargin * 2 + 2 + 1));
    ...
    if (m_controlSystemButton &&
        m_controlSystemButton->toolButtonStyle() == Qt::ToolButtonTextBesideIcon)
    {
    ...
    if (!m_controlSystemButton || m_controlSystemButton->toolButtonStyle() != Qt::ToolButtonFollowStyle)
    {
        ...
        QSize szIcon = m_frameHelper ? m_frameHelper->sizeSystemIcon(titleBarOpt.icon, titleBarOpt.rect)
                                     : QSize(16, 16);
    ...
    bool quickAccessNearTabs = m_quickAccessBar->isVisible() &&
                               m_quickAccessBarPosition == QdfRibbonBar::QATopPosition && !showTitle;
...
void QdfRibbonBarPrivate::layoutTitleButton()
{
    ...
    if (m_ribbonBarVisible)
    {
        if (m_quickAccessBarPosition != QdfRibbonBar::QATopPosition || !m_quickAccessBar->isVisible())
        {
            if (!m_controlSystemButton)
            {
                m_rcHeader.setLeft(m_rcHeader.left() +
                                   titleBarOpt.icon.actualSize(QSize(16, 16)).width() + 2);
            }
            else
            {
                if (m_controlSystemButton->toolButtonStyle() == Qt::ToolButtonFollowStyle)
                {
                    m_rcHeader.setLeft(m_rcHeader.left() + m_controlSystemButton->sizeHint().width() -
                                       5);
                }
                else
                {
    ...
    {
        int corner = 2 + (!titleBarOpt.icon.isNull() ? titleBarOpt.icon.actualSize(QSize(16, 16)).width()
                                                     : 0);
    ...
    m_actCloseButton =
            addTitleButton(QStyle::SC_TitleBarCloseButton, !dwmEnabled && buttonClose, rcButtons);
    m_actNormalButton =
            addTitleButton(QStyle::SC_TitleBarNormalButton, !dwmEnabled && buttonNormal, rcButtons);
        ...
        (bool) q->style()->styleHint((QStyle::StyleHint) QdfRibbonStyle::SH_RibbonBackstageHideTabs))
    {
        if (QdfRibbonBackstageView *backstage =
                    qobject_cast<QdfRibbonBackstageView *>(m_controlSystemButton->backstage()))
        {
...
void QdfRibbonBarPrivate::layoutContextHeaders()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        else
        {
            ...
            header->rcRect = QRect(QPoint(topLeft.x(), rectTitleText.top()),
                                   QPoint(bottomRight.x(), rectTitleText.bottom() + 1));
...
void QdfRibbonBarPrivate::layoutCorner()
{
    ...
    if (QWidget *rightWidget = q->cornerWidget(Qt::TopRightCorner))
    {
        ...
        if (rightWidget && rightWidget->isVisible())
        {
            ...
            QPoint pos(q->width() - rc.width() - hmargin + 1,
                       rcTab.top() + (rcTab.height() - rc.height()) / 2);
...
int QdfRibbonBarPrivate::indexPage(QdfRibbonPage *page) const
{
    return m_listPages.indexOf(page);
}

int QdfRibbonBarPrivate::heightTabs() const
{
    return m_heightTabs;
}
...
void QdfRibbonBarPrivate::titleBarOptions(QdfStyleOptionTitleBar &titleBarOptions) const
{
    ...
    if (m_controlSystemButton && m_controlSystemButton->isVisible())
    {
        titleBarOptions.drawIcon = !m_controlSystemButton ||
                                   m_controlSystemButton->toolButtonStyle() != Qt::ToolButtonFollowStyle;
    }
    ...
    if (!m_windowTitle.isEmpty())
    {
        ...
        titleBarOptions.text =
                titleBarOptions.fontMetrics.elidedText(m_windowTitle, Qt::ElideRight, width);
    }
        ...
        if (QdfRibbonQuickAccessBar *quickAccessBar = q->quickAccessBar())
        {
            ...
            if (actions.size() > 0)
            {
                action = popup->addAction(
                        QdfRibbonBar::tr_compatible(RibbonCustomizeQuickAccessToolBarDotString));
                action->setObjectName(strCustomizeQAToolBar);
                connect(action, SIGNAL(triggered()), this, SLOT(toggledCustomizeBar()));

                action = popup->addAction(
                        q->quickAccessBarPosition() == QdfRibbonBar::QATopPosition
...
void QdfRibbonBarPrivate::createKeyTips()
{
    ...
    for (int i = 0, count = list.count(); i < count; i++)
    {
        ...
        m_keyTips.append(new QdfRibbonKeyTip(
                q, button, strCaption,
    ...
    if (m_tabBar && m_tabBar->isVisible())
    {
        for (int i = 0, count = m_tabBar->getTabCount(); i < count; i++)
        {
            ...
            if (!strCaption.isEmpty() && tab->isEnabled() && tab->isVisible())
            {
                ...
                QdfRibbonKeyTip *keyTip = new QdfRibbonKeyTip(
                        q, tab, strCaption, tab->mapToGlobal(pt),
    ...
    if (m_controlSystemButton)
    {
        QPoint pt(m_controlSystemButton->rect().center().x(),
                  m_controlSystemButton->rect().center().y());
        ...
        m_keyTips.append(new QdfRibbonKeyTip(
                q, m_controlSystemButton, strCaption, m_controlSystemButton->mapToGlobal(pt),
    ...
    for (int i = 0, count = page->groupCount(); i < count; i++)
    {
        ...
        for (int j = 0; j < list.count(); j++)
        {
            ...
            createWidgetKeyTips(group, widget, QString(),
                                page->rect().translated(page->mapToGlobal(page->rect().topLeft())),
                                visible);
        }
    }
}
...
void QdfRibbonBarPrivate::createWidgetKeyTips(QdfRibbonGroup *group, QWidget *widget,
                                              const QString &prefix, const QRect &rcGroups, bool visible)
{
    QDF_Q(QdfRibbonBar);
    ...
    if (QAbstractButton *button = qobject_cast<QAbstractButton *>(widget))
    {
        ...
        if (strCaption.isEmpty())
        {
            if (QToolButton *toolButton = qobject_cast<QToolButton *>(button))
            {
                ...
                if (strCaption.isEmpty())
                {
                    if (QdfRibbonGallery *ribGallery =
                                qobject_cast<QdfRibbonGallery *>(toolButton->parentWidget()))
                    {
                        if (toolButton->property(_qdf_PopupButtonGallery).toBool())
                        {
                            ...
                            {
                                strCaption =
                                        ::getTextAction(*group, *toolButton->parentWidget(), propKeyTip);
                            }
                        }
                    }
    ...
    const int heightCaptionGroup = q->style()->pixelMetric(
            (QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonHeightCaptionGroup, &opt, group);
    ...
    else if ((Qt::ToolButtonStyle) widget->property("toolButtonStyle").toInt() ==
    ...
    if (menu)
    {
        ...
        for (int i = 0; i < list.count(); i++)
        {
            ...
            QdfRibbonKeyTip *keyTip =
                    new QdfRibbonKeyTip(q, menu, strCaption, menu->mapToGlobal(pt),
        ...
        for (int i = 0; i < listWidget.count(); i++)
        {
            ...
            QdfRibbonKeyTip *keyTip =
                    new QdfRibbonKeyTip(q, widget, strCaption, widget->mapToGlobal(pt),
...
int QdfRibbonBarPrivate::rowItemHeight() const
{
    return m_rowItemHeight;
}

int QdfRibbonBarPrivate::rowItemCount() const
{
...
int QdfRibbonBarPrivate::maxGroupHeight() const
{
    ...
    QStyleOption opt;
    opt.init(q);
    ret += q->style()->pixelMetric((QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonHeightCaptionGroup,
                                   &opt, nullptr) +
           4;
                ...
                else
                {
                             ...
                             keyTip->isVisibleTip())
                    {
                        ...
                        if (QAction *action = keyTip->getAction())
                        {
                            ...
                            {
                                if (QMenu *activPopup = qobject_cast<QdfRibbonSystemPopupBar *>(
                            ...
                            else
                            {
                                ...
                                {
                                    if (QWidget *activPopup = qobject_cast<QdfRibbonSystemPopupBar *>(
                                                QApplication::activePopupWidget()))
                                    {
                                        activPopup->close();
                                    }
            ...
            if (QKeyEvent *keyEvent = static_cast<QKeyEvent *>(event))
            {
                if (keyEvent->key() != Qt::Key_Escape)
                {
                    if ((qobject_cast<QMainWindow *>(obj) ||
                         (m_minimized && qobject_cast<QdfRibbonPage *>(obj))))
                    {
...
void QdfRibbonBarPrivate::createCustomizeDialog()
{
    ...
    QdfRibbonQuickAccessBarCustomizePage *quickAccessBarCustomizePage =
            new QdfRibbonQuickAccessBarCustomizePage(q);
...
{
    QDF_Q(QdfRibbonBar)
    q->setQuickAccessBarPosition(q->quickAccessBarPosition() == QdfRibbonBar::QATopPosition
                                         ? QdfRibbonBar::QABottomPosition
...
QdfRibbonPage *QdfRibbonBar::addPage(const QString &text)
{
    return insertPage(-1, text);
}

void QdfRibbonBar::addPage(QdfRibbonPage *page)
{
    insertPage(-1, page);
}
...
void QdfRibbonBar::setMinimizedFlag(bool flag)
{
    ...
    if (d->m_minimized != flag)
    {
        ...
        if (!d->m_minimized &&
            (d->m_tabBar && d->m_tabBar->getTabCount() > 0 && d->m_tabBar->currentIndex() == -1))
        {
...
void QdfRibbonBar::setMinimized(bool flag)
{
    setMinimizedFlag(flag);
}

void QdfRibbonBar::maximize()
{
    setMinimizedFlag(false);
}

bool QdfRibbonBar::isMaximized() const
{
    return !isMinimized();
}

void QdfRibbonBar::setMaximized(bool flag)
{
    setMinimizedFlag(!flag);
}
...
QAction *QdfRibbonBar::addAction(const QIcon &icon, const QString &text, Qt::ToolButtonStyle style,
                                 QMenu *menu)
{
    QDF_D(QdfRibbonBar);
...
QAction *QdfRibbonBar::addSystemButton(const QString &text)
{
...
{
    QDF_D(QdfRibbonBar);
    QAction *actionSystemPopupBar = d->m_controlSystemButton ? d->m_controlSystemButton->defaultAction()
...
bool QdfRibbonBar::isBackstageVisible() const
{
    ...
    {
        if (QdfRibbonBackstageView *backstage =
                    qobject_cast<QdfRibbonBackstageView *>(d->m_controlSystemButton->backstage()))
        {
...
void QdfRibbonBar::paintEvent(QPaintEvent *event)
{
    ...
    if (!d->m_minimized && getPageCount() > 0 && rectEvent.intersects(opt.rect))
    {
        ...
        ribbonStyle->drawControl((QStyle::ControlElement) QdfRibbonStyle::CE_RibbonGroups, &opt, &p,
                                 this);
    }
}

void QdfRibbonBar::changeEvent(QEvent *event)
{
...
void QdfRibbonBar::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if (d->m_minimizationEnabled &&
        d->m_tabBar->getTab(d->m_tabBar->currentIndex()) == childAt(event->pos()))
    {
