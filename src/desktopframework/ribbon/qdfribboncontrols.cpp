...
void QdfRibbonControlSizeDefinitionPrivate::init(QdfRibbonControl *parentControl,
                                                 QdfRibbonControlSizeDefinition::GroupSize size)
{
    m_parentControl = parentControl;
    if (size == QdfRibbonControlSizeDefinition::GroupLarge ||
        size == QdfRibbonControlSizeDefinition::GroupPopup)
    {
...
QdfRibbonControlSizeDefinition::QdfRibbonControlSizeDefinition(
        QdfRibbonControl *parentControl, QdfRibbonControlSizeDefinition::GroupSize size)
    : QObject()
{
...
void QdfRibbonControlPrivate::init(QdfRibbonGroup *parentGroup)
{
    ...
    m_allSizes.append(
            new QdfRibbonControlSizeDefinition(q, QdfRibbonControlSizeDefinition::GroupLarge));// Large
    m_allSizes.append(
            new QdfRibbonControlSizeDefinition(q, QdfRibbonControlSizeDefinition::GroupMedium));// Medium
    m_allSizes.append(
            new QdfRibbonControlSizeDefinition(q, QdfRibbonControlSizeDefinition::GroupSmall));// Small
    m_allSizes.append(
            new QdfRibbonControlSizeDefinition(q, QdfRibbonControlSizeDefinition::GroupPopup));// Popup
}
...
QdfRibbonControlSizeDefinition *
QdfRibbonControl::sizeDefinition(QdfRibbonControlSizeDefinition::GroupSize size) const
{
    QDF_D(const QdfRibbonControl);
    return d->m_allSizes[(int) size];
}

void QdfRibbonControl::adjustCurrentSize(bool expand)
{
    Q_UNUSED(expand);
}
...
QdfRibbonWidgetControl::~QdfRibbonWidgetControl()
{
    QDF_FINI_PRIVATE();
}
...
void QdfRibbonWidgetControl::paintEvent(QPaintEvent *event)
{
    ...
    if (!d->m_ignoreActionSettings && d->m_widget != nullptr)
    {
        ...
        if (!opt.text.isEmpty())
        {
            ...
            style()->drawItemText(&painter, rectText, flags, opt.palette,
...
QdfRibbonColumnBreakControl::QdfRibbonColumnBreakControl(QdfRibbonGroup *parentGroup)
...
QdfRibbonColumnBreakControl::~QdfRibbonColumnBreakControl()
{
    QDF_FINI_PRIVATE();
}
...
QSize QdfRibbonColumnBreakControl::sizeHint() const
{
    ...
    d->initStyleOption(&opt);
    const int height =
            parentGroup()->ribbonBar()->rowItemHeight() * parentGroup()->ribbonBar()->rowItemCount();
...
QLabel *QdfRibbonLabelControl::widget() const
{
    return qobject_cast<QLabel *>(contentWidget());
}

/*QdfRibbonToolBarSeparatorControlPrivate*/
QdfRibbonToolBarSeparatorControlPrivate::QdfRibbonToolBarSeparatorControlPrivate(
        QdfRibbonToolBarControl *parent)
    : QWidget(parent)
{
...
{
    bool endgroup = m_groups.size() == 0 || (m_groups.last().items.size() > 0 &&
                                             m_groups.last().items.last().action->isSeparator());
    if (endgroup)
    {
...
static int indexOfAction(const QdfRibbonToolBarControlPrivate::RibbonToolBarActionGroupItems &group,
                         QAction *action)
{
    int index = 0;
    for (QdfRibbonToolBarControlPrivate::RibbonToolBarActionGroupItems::const_iterator it =
                 group.constBegin();
...
void QdfRibbonToolBarControlPrivate::updateGroupsGeometry(const QRect &rect)
{
    ...
    if (isMixedWidgets())
    {
        for (RibbonToolBarActionGroupList::iterator it = m_groups.begin(); it != m_groups.end(); ++it)
        {
            ...
            {
                QRect rectElem(
                        QPoint(m_rect.left() + x + m_margin, m_rect.top() /*+ m_margin / 2*/),
    ...
    for (RibbonToolBarActionGroupList::iterator it = m_groups.begin(); it != m_groups.end(); ++it)
    {
        ...
        QRect rectElem(QPoint(m_rect.left() + x + m_margin, m_rect.top() + y /*+ m_margin / 2*/),
                       sizeElem);
    ...
    if (m_calcRows > 1 && !m_preserveElementOrder)
    {
        ...
        while (true)
        {
            ...
            for (RibbonToolBarActionGroupList::iterator it = m_groups.begin(); it != m_groups.end();
                 ++it)
            {
                RibbonToolBarActionsGroup &actionGroup = *it;
            ...
            for (i = maxRowIndex + 1; i < vecRowWidths.size(); i++)
            {
                if (vecRowWidths[i] + minWidth < maxRowWidth)
                {
                    ...
                    for (RibbonToolBarActionGroupList::iterator it = m_groups.begin();
                         it != m_groups.end(); ++it)
                    {
                        RibbonToolBarActionsGroup &actionGroup = *it;
...
void QdfRibbonToolBarControlPrivate::updateGeometryWidget()
{
    ...
    for (RibbonToolBarActionGroupList::iterator it = m_groups.begin(); it != m_groups.end(); ++it)
    {
        ...
        int x = rect.left();
        for (QList<RibbonToolBarActionGroupItem>::iterator itItem = items.begin(); itItem != items.end();
             ++itItem)
        {
            ...
            if (item.widget)
            {
                ...
                if (lastWidget)
                {
                    ...
                    {
                        sizeElem.setHeight(sizeElem.height() < rowHeight ? rowHeight
                                                                         : sizeElem.height());
                    }
                }
                else
         ...
         it != group.constEnd(); ++it)
    {
        if (it->action->isSeparator())
        {
            if (it->action->isVisible())
            {
                QStyleOption opt;
                opt.init(q);
                const int widthSeparator = q->parentGroup()->style()->pixelMetric(
                        QStyle::PM_ToolBarSeparatorExtent, &opt, q);
        ...
        if (/*it->action->isVisible()*/ !it->isEmpty())
        {
            ...
            s.setHeight(s.height() < ribbonBar->rowItemHeight() ? ribbonBar->rowItemHeight()
                                                                : s.height());
            ret.rwidth() += s.width();
...
void QdfRibbonToolBarControlPrivate::updateLayoutParent(bool calcLayoutParent)
{
    ...
    for (RibbonToolBarActionGroupList::iterator itGroup = m_groups.begin(); itGroup != m_groups.end();
         ++itGroup)
    {
        RibbonToolBarActionsGroup &group = *itGroup;
        for (QdfRibbonToolBarControlPrivate::RibbonToolBarActionGroupItems::iterator itItem =
                     group.items.begin();
...
QdfRibbonToolBarControl::QdfRibbonToolBarControl(QdfRibbonGroup *parentGroup)
    : QdfRibbonControl(parentGroup)
{
    QDF_INIT_PRIVATE(QdfRibbonToolBarControl);
}

QdfRibbonToolBarControl::~QdfRibbonToolBarControl()
{
    QDF_FINI_PRIVATE();
}
                                          ...
                                          QToolButton::ToolButtonPopupMode mode)
{
    ...
    Q_ASSERT_X(mode != QToolButton::DelayedPopup, "QdfRibbonToolBarControl::addMenu",
               "The use of mode is not valid");
...
QAction *QdfRibbonToolBarControl::insertAction(QAction *before, QAction *action,
...
QAction *QdfRibbonToolBarControl::addAction(const QString &text, const QObject *receiver,
                                            const char *member)
{
    QAction *action = new QAction(text, this);
...
void QdfRibbonToolBarControl::updateLayout()
{
    ...
    if (QdfRibbonBar *ribbonBar = parentGroup()->ribbonBar())
    {
        ...
        const int height = (ribbonBar->rowItemHeight() * ribbonBar->rowItemCount()) +
                           2 * (d->m_margin / 2) + vertMargin;
        sz.setHeight(height);
        ...
        case QEvent::ActionAdded:
            {
                ...
                {
                    QdfRibbonToolBarSeparatorControlPrivate *separator =
                            new QdfRibbonToolBarSeparatorControlPrivate(this);
                    widget = separator;
                }
