                                                 ...
                                                 const QModelIndex &index) const
{
    ...
    if (!opt.text.isEmpty())
    {
        ...
        style->drawItemText(painter, textRect,
                            QStyle::visualAlignment(opt.direction, opt.displayAlignment), opt.palette,
                            true /*enabled*/,
...
void QdfRibbonQuickAccessBarCustomizePagePrivate::init()
{
...
#endif
    QSize sz = QSize(9, 9);
    QPixmap m_imagesUpSrc = imagesSrc.copy(
            QRect(QPoint((int) QdfOfficePaintManager::Icon_ArowUpLarge * sz.width(), 0), sz));
    QPixmap m_imagesDownSrc = imagesSrc.copy(
            QRect(QPoint((int) QdfOfficePaintManager::Icon_ArowDownLarge * sz.width(), 0), sz));
    ...
    connect(ui->quickAccessBarList, SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)),
            this, SLOT(currentQTCommandChanged(QListWidgetItem *)));
...
QPixmap QdfRibbonQuickAccessBarCustomizePagePrivate::createIconExStyleWidget(QWidget *widget,
                                                                             bool &iconView)
{
    return QdfDrawHelpers::createIconExStyleWidget(widget, m_widthColViewItem, m_heightRowItem,
                                                   iconView);
}
...
void QdfRibbonQuickAccessBarCustomizePagePrivate::fillListCategorieCommads(
...
void QdfRibbonQuickAccessBarCustomizePagePrivate::applyClicked()
{
    ...
    {
        m_ribbonBar->setQuickAccessBarPosition(ui->checkBoxQATOn->isChecked()
...
QdfRibbonQuickAccessBarCustomizePage::QdfRibbonQuickAccessBarCustomizePage(QdfRibbonBar *ribbonBar)
    : QWidget(nullptr)
{
...
QdfRibbonQuickAccessBarCustomizePage::~QdfRibbonQuickAccessBarCustomizePage()
{
    QDF_FINI_PRIVATE();
}
...
void QdfRibbonBarCustomizePagePrivate::setCategoryCaptionTree(QTreeWidget *tree,
...
void QdfRibbonBarCustomizePagePrivate::fillSourceActions(QList<QAction *> &actions,
...
void QdfRibbonBarCustomizePagePrivate::fillSourcePages(QList<QdfRibbonPage *> &pages,
                                                       const QString &strCategory)
{
...
void QdfRibbonBarCustomizePagePrivate::fillSourceGroupActions(QdfRibbonGroup *group,
                                                              QTreeWidgetItem *parentItem)
{
...
void QdfRibbonBarCustomizePagePrivate::fillStateRibbon(const QString &strCategory)
{
    ...
    while (itPages.hasNext())
    {
        ...
        {
            QString pageTitle =
                    !manager->pageName(page).isEmpty() ? manager->pageName(page) : page->title();
...
void QdfRibbonBarCustomizePagePrivate::fillStateGroups(QdfRibbonPage *page, QTreeWidgetItem *itemParent)
{
    ...
    while (itGroups.hasNext())
    {
        ...
        QString groupTitle =
                !manager->groupName(group).isEmpty() ? manager->groupName(group) : group->title();
...
void QdfRibbonBarCustomizePagePrivate::fillStateActions(QdfRibbonGroup *group,
                                                        QTreeWidgetItem *itemParent, bool newGroup)
{
    Q_ASSERT(group != nullptr);
    QdfRibbonCustomizeManager *manager = m_ribbonBar->customizeManager();
    Q_ASSERT(manager != nullptr);
    ...
    bool isCustomGroup =
            group->property(__qdf_Widget_Custom).toString() == QLatin1String("__qtc_Group_Custom");
...
void QdfRibbonBarCustomizePagePrivate::addActionGroupCustom(QdfRibbonGroup *group, QAction *action,
                                                            int index)
{
    QdfRibbonCustomizeManager *manager = m_ribbonBar->customizeManager();
    Q_ASSERT(manager != nullptr);
...
void QdfRibbonBarCustomizePagePrivate::setButtons()
{
    ...
    else if (m_currentItemPage)
    {
        ...
        QdfRibbonPage *currentPage = m_currentItemToPage.value(m_currentItemPage);
        if (currentPage &&
            currentPage->property(__qdf_Widget_Custom).toString() == QLatin1String("__qtc_Page_Custom"))
        {
            removeEnabled = true;
        }
    }
...
void QdfRibbonBarCustomizePagePrivate::cancelClicked()
{
    ...
    if (m_currentSourceAction && m_currentItemGroup)
    {
            ...
            if (QAction *action = m_currentSourceItemToAction.value(m_currentSourceAction))
            {
                ...
                else
                {
                    QTreeWidgetItem *currentItem =
                            m_currentItemGroup->child(m_currentItemGroup->childCount() - 1);
        ...
        else
        {
            QMessageBox::warning(q, q->windowTitle(),
                                 QdfRibbonBar::tr_compatible(RibbonAddCommandWarningString));
        }
    }
    else if (!m_currentSourceItemGroup)
    {
...
template<class T>
QTreeWidgetItem *currentItem(const QMap<QTreeWidgetItem *, T *> &currentItemToObject,
...
QdfRibbonBarCustomizePage::~QdfRibbonBarCustomizePage()
{
    QDF_FINI_PRIVATE();
}
...
void QdfRibbonBarCustomizePage::hideEvent(QHideEvent *event)
{
