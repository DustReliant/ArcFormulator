...
QdfRibbonSliderButton::QdfRibbonSliderButton(QWidget *parent, QStyle::PrimitiveElement typeBut)
    : QPushButton(parent)
{
    m_typeBut = typeBut;
}
...
void QdfRibbonSliderButton::paintEvent(QPaintEvent *event)
{
    ...
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonSliderButton, &option,
                           &painter, this);
}
...
QSize QdfRibbonSliderButton::sizeHint() const
{
    ...
    initStyleOption(&option);
    option.direction = m_typeBut == QStyle::PE_IndicatorArrowUp ? Qt::LeftToRight : Qt::RightToLeft;
    sz = style()->sizeFromContents((QStyle::ContentsType) QdfRibbonStyle::CT_RibbonSliderButton, &option,
                                   sz, this);
    return sz;
}
...
QdfRibbonSliderPane::~QdfRibbonSliderPane()
{
    QDF_FINI_PRIVATE();
}
