...
QdfRibbonToolTip::QdfRibbonToolTip(const QString &title, const QString &text, const QIcon &icon,
                                   QWidget *parent)
...
QdfRibbonToolTip *QdfRibbonToolTip::instance()
{
    return QdfRibbonToolTipPrivate::m_instance;
}

void QdfRibbonToolTip::showToolTip(const QPoint &pos, const QString &title, const QString &text,
                                   const QIcon &icon, QWidget *w)
{
    QRect rect;
    ...
    if (!text.isEmpty())
    {
        ...
        new QdfRibbonToolTip(title, text, icon,
                             QApplication::desktop()->screen(QdfRibbonToolTip::getTipScreen(pos, w)));
#endif
        QdfRibbonToolTipPrivate::m_instance->setTipRect(w, rect);
        QdfRibbonToolTipPrivate::m_instance->placeTip(pos, w);
...
void QdfRibbonToolTip::hideToolTip()
{
...
{
    return (QdfRibbonToolTipPrivate::m_instance != 0 &&
            QdfRibbonToolTipPrivate::m_instance->isVisible());
}

void QdfRibbonToolTip::setWrapMode(WrapMode mode)
{
    QdfRibbonToolTipPrivate::m_wrapMode = mode;
}

QdfRibbonToolTip::WrapMode QdfRibbonToolTip::wordWrap()
{
    return QdfRibbonToolTipPrivate::m_wrapMode;
}

void QdfRibbonToolTip::setIconVisible(bool visible)
{
    QdfRibbonToolTipPrivate::m_showIcon = visible;
}

bool QdfRibbonToolTip::isIconVisible()
{
    return QdfRibbonToolTipPrivate::m_showIcon;
}
...
QSize QdfRibbonToolTip::sizeHint() const
{
    ...
    QSize szMargin(3 + rcMargin.left() + rcMargin.right() + 3,
                   3 + rcMargin.top() + rcMargin.bottom() + 3);
    ...
    if (!d->m_label)
    {
        ...
        rcText = fm.boundingRect(rcText.left(), rcText.top(), rcText.width(), rcText.height(),
                                 flags | Qt::TextWordWrap, d->m_text);
        szText = rcText.size();
    }
    else
    {
...
void QdfRibbonToolTip::paintEvent(QPaintEvent *event)
{
    ...
    if (drawTitle)
    {
        ...
        fnt.setBold(true);
        p.setFont(fnt);
        style()->drawItemText(&p, rcTitle, flags | Qt::TextSingleLine, opt.palette, isEnabled(),
                              d->m_textTitle, QPalette::ToolTipText);
        p.setFont(oldFont);

        rc.setTop(rc.top() +
                  p.boundingRect(rcTitle, flags | Qt::TextSingleLine, d->m_textTitle).height());
        rc.adjust(0, 12, 0, 0);
    }
...
bool QdfRibbonToolTip::eventFilter(QObject *o, QEvent *event)
{
    ...
    switch (event->type())
    {
        ...
        case QEvent::MouseMove:
            if (o == d->m_widget && !d->m_rect.isNull() &&
                !d->m_rect.contains(static_cast<QMouseEvent *>(event)->pos()))
            {
                hideTip();
            }
        default:
...
bool QdfRibbonToolTip::tipChanged(const QPoint &pos, const QString &text, const QString &textTitle,
                                  QObject *o)
{
    QDF_D(QdfRibbonToolTip);
...
QdfRibbonKeyTip::QdfRibbonKeyTip(QWidget *bars, QWidget *owner, const QString &caption, const QPoint &pt,
                                 uint align, bool enabled, QAction *action)
...
QdfRibbonKeyTip::~QdfRibbonKeyTip()
{
    QDF_FINI_PRIVATE();
}
...
void QdfRibbonKeyTip::paintEvent(QPaintEvent *event)
{
    ...
    style()->drawItemText(&p, opt.rect,
                          Qt::AlignVCenter | Qt::AlignCenter | Qt::TextSingleLine | Qt::TextWordWrap,
