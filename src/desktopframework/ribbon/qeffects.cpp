...
QAlphaWidget::QAlphaWidget(QWidget *w, Qt::WindowFlags f) : QWidget(effectParent(w), f)
{
#ifndef Q_OS_WIN
    setEnabled(false);
#endif
...
void QAlphaWidget::run(int time)
{
    ...
    backImage = QGuiApplication::primaryScreen()
                        ->grabWindow(QApplication::desktop()->winId(), widget->geometry().x(),
    ...
    if (!done && widget)
    {
        ...
        if (currentWidth != totalWidth)
        {
            currentWidth = totalWidth * (elapsed / duration) +
                           (2 * totalWidth * (elapsed % duration) + duration) / (2 * duration);
        ...
        if (currentHeight != totalHeight)
        {
            currentHeight = totalHeight * (elapsed / duration) +
                            (2 * totalHeight * (elapsed % duration) + duration) / (2 * duration);
