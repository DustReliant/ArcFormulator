...
typedef BOOL(WINAPI *PtrDwmDefWindowProc)(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
                                          LRESULT *plResult);
...
typedef HRESULT(WINAPI *PtrSetWindowThemeAttribute)(HWND hwnd,
                                                    enum HLP_WINDOWTHEMEATTRIBUTETYPE eAttribute,
...
typedef HRESULT(WINAPI *PtrDrawThemeTextEx)(HANDLE hTheme, HDC hdc, int iPartId, int iStateId,
                                            LPCWSTR pszText, int cchText, DWORD dwTextFlags,
                                            LPRECT pRect, const HLP_DTTOPTS *pOptions);
typedef HRESULT(WINAPI *PtrDrawThemeIcon)(HANDLE hTheme, HDC hdc, int iPartId, int iStateId,
    ...
    if (m_pSetWinEventHook && m_pUnhookWinEvent)
    {
        m_hWinEventHook =
                m_pSetWinEventHook(EVENT_OBJECT_REORDER, EVENT_OBJECT_REORDER, nullptr,
                                   &QdfOfficeFrameHelperWin::FrameHelperEventHook::WinEventProc,
...
void CALLBACK QdfOfficeFrameHelperWin::FrameHelperEventHook::WinEventProc(
...
bool QdfOfficeFrameHelperWin::isActive() const
{
    return m_active;
}

bool QdfOfficeFrameHelperWin::isMaximize() const
{
    return getStyle() & WS_MAXIMIZE;
}

bool QdfOfficeFrameHelperWin::canHideTitle() const
{
    return true;
}

int QdfOfficeFrameHelperWin::frameBorder() const
{
...
bool QdfOfficeFrameHelperWin::clientMetrics(int &iCaptionWidth, int &iCaptionHeight) const
{
    ...
    const UINT cbProperSize = (comCtlVersion() < MAKELONG(1, 6)) ? sizeof(QTC_OLDNONCLIENTMETRICS)
...
void QdfOfficeFrameHelperWin::setHeaderRect(const QRect &rcHeader)
{
    m_rcHeader = rcHeader;
}
...
void QdfOfficeFrameHelperWin::updateFrameRegion(const QSize &szFrameRegion, bool bUpdate)
{
    ...
    if (m_szFrameRegion != szFrameRegion || bUpdate)
    {
        ...
        if (!m_dwmEnabled)
        {
            ...
            if (!isMDIMaximized())
            {
                ...
                else
                {
                    if (true)
                    {
                        hRgn = ::CreateRectRgn(0, 0, szFrameRegion.width() + 1,
                                               szFrameRegion.height() + 1);
                    }
                    else
                    {
        ...
        else
        {
            ...
            const bool flatFrame = (bool) m_ribbonBar->style()->styleHint(
                    (QStyle::StyleHint) QdfRibbonStyle::SH_FlatFrame);
...
void QdfOfficeFrameHelperWin::initStyleOption(QdfStyleOptionFrame *option)
{
    ...
    option->clientRect =
            QRect(QPoint(rcClient.left, rcClient.top), QPoint(rcClient.right, rcClient.bottom));
    option->maximizeFlags = getStyle() & WS_MAXIMIZE;
    option->isBackstageVisible = m_ribbonBar->isBackstageVisible() &&
...
static void drawPartFrame(QWidget *frame, HDC hdc, HDC cdc, const QdfStyleOptionFrame &opt,
                          const QRect &rect)
{
...
void QdfOfficeFrameHelperWin::redrawFrame()
{
    if (!m_dwmEnabled)
    {
        ...
        else
        {
            ...
            // draw top side
            ::drawPartFrame(m_frame, hdc, cdc, opt,
                            QRect(0, 0, width, opt.titleVisible ? opt.titleBarSize : fBorder));
            // draw right side
            ...
            // draw bottom side
            ::drawPartFrame(m_frame, hdc, cdc, opt,
                            QRect(0, height - borderSizeBotton, width, borderSizeBotton));
            ::DeleteDC(cdc);
            ...
            if (opt.titleVisible)
            {
...
#endif
                {
                    ...
                    ::DrawIconEx(hdc, rect.left(), rect.top(), hIcon, 0, 0, 0, nullptr,
                                 DI_NORMAL | DI_COMPAT);
                    ::DestroyIcon(hIcon);
                }
                ...
                if (!opt.active)
                {
                    Qdf::QdfStyleHintReturnThemeColor hintReturnInActive("Window",
                                                                         "CaptionTextInActive");
                    clrFrameCaptionText = static_cast<QRgb>(m_frame->style()->styleHint(
                            QStyle::SH_CustomBase, nullptr, nullptr, &hintReturnInActive));
                }
                else
                {
                    Qdf::QdfStyleHintReturnThemeColor hintReturnActive("Window", "CaptionTextActive");
                    clrFrameCaptionText = static_cast<QRgb>(m_frame->style()->styleHint(
                            QStyle::SH_CustomBase, nullptr, nullptr, &hintReturnActive));
                }

                COLORREF colorInActive = RGB(qRed(clrFrameCaptionText), qGreen(clrFrameCaptionText),
                                             qBlue(clrFrameCaptionText));
                ...
                QSize szIcon = sizeSystemIcon(QIcon(), QRect());
                RECT rc = {nFrameBorder + szIcon.width() + 3, nTopBorder,
                           opt.rect.width() - nFrameBorder /*-100*/, opt.titleBarSize};
...
void QdfOfficeFrameHelperWin::drawTitleBar(QPainter *painter, const QdfStyleOptionTitleBar &opt)
{
    ...
    wchar_t *text = new wchar_t[len + 1];
    GetWindowTextW(m_hwndFrame, text, len + 1);
    drawDwmCaptionText(painter, opt.rcTitleText, text[0] != 0 ? QString::fromWCharArray(text) : opt.text,
                       opt.clrText, opt.state & QStyle::State_Active, false);
    delete[] text;
    ...
    if (opt.drawIcon && !opt.icon.isNull())
    {
        ...
        if (HICON hIcon = windowIcon(opt.icon, opt.rect))
        {
            ...
            {
                RECT rcIcon = {rect.left(), rect.top(), rect.width() + rect.left(),
                               rect.height() + rect.top()};
                HIMAGELIST hImageList = ::ImageList_Create(szIcon.width(), szIcon.height(),
...
void QdfOfficeFrameHelperWin::fillSolidRect(QPainter *painter, const QRect &rect,
                                            const QRegion &airRegion, QColor clr)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
    Q_UNUSED(painter);
    QPlatformNativeInterface *nativeInterface = QGuiApplication::platformNativeInterface();
...
void QdfOfficeFrameHelperWin::drawDwmCaptionText(QPainter *painter, const QRect &rc,
                                                 const QString &strWindowText, const QColor &colText,
                                                 bool active, bool contextTab)
{
    ...
    pDrawThemeTextEx(hTheme, dcMem, maximizeStyle ? HLP_WP_MAXCAPTION : HLP_WP_CAPTION,
                     active ? HLP_FS_ACTIVE : HLP_FS_INACTIVE, (LPCWSTR) text.utf16(), -1, uFormat,
                     &rctext, &dto);
...
#endif
    painter->drawPixmap(rect.left(), rect.top() + (maximizeStyle ? m_frameBorder * 0.5 : m_frameBorder),
                        pxx);
...
HRGN QdfOfficeFrameHelperWin::calcFrameRegion(QSize sz)
{
    ...
    memcpy(&pRgnData->Buffer + sizeTopRect,
           hasStatusBar ? (void *) &rgnRibbonBottomFrame : (void *) &rgnSimpleBottomFrame,
           sizeBottomRect);
...
void QdfOfficeFrameHelperWin::collapseTopFrame()
{
    ...
    if (QWindow *window = m_frame->windowHandle())
    {
        if (QPlatformWindow *platformWindow = window->handle())
        {
            ...
            int valTitle = m_dwmEnabled
                                   ? (visible ? captionSize() + (m_frameBorder * 2) : 0)
...
bool QdfOfficeFrameHelperWin::winEvent(MSG *msg, long *result)
{
    if (m_hwndFrame && msg->hwnd != m_hwndFrame &&
        (m_ribbonBar && m_ribbonBar->testAttribute(Qt::WA_NativeWindow) &&
         msg->hwnd == (HWND) m_ribbonBar->winId()))
    {
        if (msg->message == WM_NCHITTEST)
        {
            ...
            if (::PtInRect(&rc, point))
            {
                ...
                if (lResult == HTMINBUTTON || lResult == HTMAXBUTTON || lResult == HTCLOSE ||
                    lResult == HTHELP || lResult == HTCLIENT)
                {
    ...
    if (!m_dwmEnabled && m_lockNCPaint &&
        (msg->message == WM_STYLECHANGING || msg->message == WM_STYLECHANGED ||
    ...
    else if (msg->message == WM_SETTINGCHANGE)
    {
        ...
        if (uFlags == SPI_SETNONCLIENTMETRICS || uFlags == SPI_SETWORKAREA ||
            ...
            *result = DefWindowProcW(msg->hwnd, msg->message, msg->wParam, msg->lParam);
            ...
            if (*result == HTCLIENT)
            {
                ...
                if (*result == HTCLIENT)
                {
                    ...
                    {
                        if (!hitTestContextHeaders(pos) && !hitTestBackstageHeaders(pos) &&
                            m_rcHeader.contains(pos))
                        {
                            *result = HTCAPTION;
                        }
                    }
                    ...
                    {
                        DWORD dwStyle = getStyle();
                        if (pos.x() < 7 + sizeSystemIcon(QIcon(), QRect()).width() &&
                            (dwStyle & WS_SYSMENU))
                        {
                            *result = HTSYSMENU;
                        }
    ...
    else if (m_hwndFrame != nullptr && msg->message == WM_NCHITTEST && !isTitleVisible() && m_dwmEnabled)
    {
        ...
        if (*result == HTCLIENT || *result == HTSYSMENU)
        {
            ...
            if (::PtInRect(&rc, point))
            {
                ...
                DWORD dwStyle = getStyle();
                if ((point.y < rc.top + m_frameBorder) && (dwStyle & WS_SIZEBOX) &&
                ...
                if (*result != HTSYSMENU)
                {
                    *result = HTCAPTION;
                    DWORD dwStyle = getStyle();
                    if (pos.x() < m_frameBorder + sizeSystemIcon(QIcon(), QRect()).width() &&
                        (dwStyle & WS_SYSMENU))
                    {
                        *result = HTSYSMENU;
                    }
    ...
    else if (msg->message == WM_SYSCOMMAND && !m_dwmEnabled && msg->wParam == SC_MINIMIZE &&
             getStyle() & WS_CHILD)
    {
        *result = DefWindowProcW(msg->hwnd, msg->message, msg->wParam, msg->lParam);
    ...
    else if (m_hwndFrame != nullptr && msg->message == WM_SHOWWINDOW)
    {
        ...
        if (!bShow)
        {
            ...
            int valTitle =
                    m_dwmEnabled ? (visible ? captionSize() + (m_frameBorder * 2) : 0)
    ...
    else if (msg->message == WM_SHOWWINDOW)
    {
        ...
        if (!bShow)
        {
            ...
            int valTitle = m_dwmEnabled ? (visible ? captionSize() + (m_frameBorder * 2) : 0)
                                        : (visible ? captionSize() + m_frameBorder / 2 - 1
                                                   : m_frameBorder / 2 - 1);
    ...
    if (obj == m_frame)
    {
        ...
        if (event->type() == QEvent::Resize)
        {
            ...
            else if (event->type() == QEvent::StyleChange)
            {
                ...
                {
                    m_frame->setWindowState(
                            (m_frame->windowState() & ~(Qt::WindowMinimized | Qt::WindowFullScreen)) |
                            Qt::WindowMaximized);
                }
#endif// QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
            }
...
bool QdfOfficeFrameHelperWin::isSmallSystemBorders()
{
    return true;
}
...
bool QdfOfficeFrameHelperWin::setWindowAttribute(HWND hwnd, DWORD dwAttribute, LPCVOID pvAttribute,
                                                 DWORD cbAttribute)
{
...
bool QdfRibbonPaintManager::drawFrame(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    ...
    if (qobject_cast<const QdfMainWindow *>(w))
    {
        if (const QdfStyleOptionFrame *optFrame = qstyleoption_cast<const QdfStyleOptionFrame *>(opt))
        {
            ...
            COLORREF clrBorder = optFrame->active ? rgbcolorref(d->m_clrFrameBorderActive0)
                                                  : rgbcolorref(d->m_clrFrameBorderInactive0);
            ...
            if (optFrame->hasStatusBar && !optFrame->isBackstageVisible)
            {
                ...
                RECT rectBottom = {0, rc.height() - nBottomBorder, rc.width(),
                                   rc.height() + nBottomBorder};
                ::FillRect(hdc, &rectBottom, hBrushStatusBar);

                RECT rectLeft = {
                        0, rc.height() - (statusHeight + (int) QdfDrawHelpers::dpiScaled(nTopBorder)),
                        nLeftBorder, rc.height() - nBottomBorder};
                ::FillRect(hdc, &rectLeft, hBrushStatusBar);

                RECT rectRight = {rc.width() - nRightBorder,
                                  rc.height() -
                                          (statusHeight + (int) QdfDrawHelpers::dpiScaled(nTopBorder)),
                                  rc.width(), rc.height() - nBottomBorder};
