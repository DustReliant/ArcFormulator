#ifndef DESKTOPFRAMEWORK_QDFOFFICEFRAMEHELPER_WIN_H
#define DESKTOPFRAMEWORK_QDFOFFICEFRAMEHELPER_WIN_H
...
class QdfStyleOptionFrame;
class QdfRibbonBar;

class QdfOfficeFrameHelperWin : public QObject, public QdfOfficeFrameHelper
{
    Q_OBJECT
public:
    QdfOfficeFrameHelperWin(QWidget *parent = nullptr);
    virtual ~QdfOfficeFrameHelperWin();
...
public:
    ...
    virtual void setContextHeader(QList<QdfContextHeader *> *listContextHeaders);
    ...
    virtual void drawTitleBar(QPainter *painter, const QdfStyleOptionTitleBar &opt);
    virtual void fillSolidRect(QPainter *painter, const QRect &rect, const QRegion &airRegion,
                               QColor clr);
...
protected:
    ...
    void initStyleOption(QdfStyleOptionFrame *option);
    void redrawFrame();
    bool modifyStyle(HWND hWnd, DWORD dwRemove, DWORD dwAdd, UINT nFlags);
    bool isFrameHasStatusBar(int *statusHeight = nullptr) const;
...
private:
    ...
    Q_DISABLE_COPY(QdfOfficeFrameHelperWin)
};

QDF_END_NAMESPACE

#endif//DESKTOPFRAMEWORK_QDFOFFICEFRAMEHELPER_WIN_H