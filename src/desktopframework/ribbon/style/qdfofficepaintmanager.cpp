...
static inline bool isAncestor(const QObject *child)
{
    while (child)
    {
        if (qobject_cast<const QdfRibbonBar *>(child))
        {
            return true;
        }
        child = child->parent();
...
QdfOfficePaintManager::QdfOfficePaintManager(QdfCommonStyle *baseStyle)
    : QdfCommonPaintManager(baseStyle)
{
    loadPixmap();
...
QdfOfficePaintManager::~QdfOfficePaintManager()
{
}

QdfOfficeStyle::Theme QdfOfficePaintManager::theme() const
{
    return ((QdfOfficeStyle *) baseStyle())->getTheme();
}

QString QdfOfficePaintManager::theme(const QString &str) const
{
    return ((QdfOfficeStyle *) baseStyle())->qdf_d()->theme(str);
}

QColor QdfOfficePaintManager::textColor(bool selected, bool pressed, bool enabled, bool checked,
                                        bool popuped, BarType barType, BarPosition barPosition) const
{
    return ((QdfOfficeStyle *) baseStyle())
            ->getTextColor(selected, pressed, enabled, checked, popuped, barType, barPosition);
}

void QdfOfficePaintManager::polish(QWidget *widget)
{
    QDF_D_STYLE(QdfOfficeStyle);
...
QIcon QdfOfficePaintManager::standardIconEx(QStyle::StandardPixmap px, const QStyleOption *opt,
...
bool QdfOfficePaintManager::drawWorkspace(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    Q_UNUSED(w);
...
bool QdfOfficePaintManager::drawToolBar(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    Q_UNUSED(w);
...
QColor QdfOfficePaintManager::menuLabelColor() const
{
    return m_clrBarShadow;
}

void QdfOfficePaintManager::drawSysButton(QPainter *p, const QRect &rect, bool down,
                                          bool highlighted) const
{
    QDF_D_STYLE(const QdfOfficeStyle);
    QColor color;
    if (down)
    {
    ...
    if (down && highlighted)
    {
        QdfDrawHelpers::draw3dRectEx(*p, rect, m_clrHighlightDn, m_clrHighlightDn);
    }
}

bool QdfOfficePaintManager::drawTitleBar(const QStyleOptionComplex *opt, QPainter *p,
                                         const QWidget *wd) const
{
    if (!wd || !qobject_cast<const QMdiSubWindow *>(wd))
    {
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
        QWidget *widget = (wd && wd->parentWidget()) ? wd->parentWidget()->parentWidget() : nullptr;
    ...
    if (const QStyleOptionTitleBar *tb = qstyleoption_cast<const QStyleOptionTitleBar *>(opt))
    {
        ...
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb, QStyle::SC_TitleBarLabel,
                                                      wd);
        ...
        if (tb->subControls & QStyle::SC_TitleBarCloseButton &&
            tb->titleBarFlags & Qt::WindowSystemMenuHint)
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarCloseButton, wd);
            down = tb->activeSubControls & QStyle::SC_TitleBarCloseButton &&
                   (opt->state & QStyle::State_Sunken);
            highlighted = tb->activeSubControls & QStyle::SC_TitleBarCloseButton &&
                          (opt->state & QStyle::State_MouseOver);

            if ((tb->titleBarFlags & Qt::WindowType_Mask) == Qt::Tool)
            {
                pm = baseStyle()
                             ->standardIcon(QStyle::SP_DockWidgetCloseButton, &tool, wd)
                             .pixmap(10, 10);
            }
            else
            {
        ...
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarMaxButton, wd);
            down = tb->activeSubControls & QStyle::SC_TitleBarMaxButton &&
                   (opt->state & QStyle::State_Sunken);
            highlighted = tb->activeSubControls & QStyle::SC_TitleBarMaxButton &&
                          (opt->state & QStyle::State_MouseOver);
        ...
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarMinButton, wd);
            down = tb->activeSubControls & QStyle::SC_TitleBarMinButton &&
                   (opt->state & QStyle::State_Sunken);
            highlighted = tb->activeSubControls & QStyle::SC_TitleBarMinButton &&
                          (opt->state & QStyle::State_MouseOver);
        ...
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarNormalButton, wd);
            down = tb->activeSubControls & QStyle::SC_TitleBarNormalButton &&
                   (opt->state & QStyle::State_Sunken);
            highlighted = tb->activeSubControls & QStyle::SC_TitleBarNormalButton &&
                          (opt->state & QStyle::State_MouseOver);
        ...
        if (tb->subControls & QStyle::SC_TitleBarShadeButton &&
            tb->titleBarFlags & Qt::WindowShadeButtonHint && !(tb->titleBarState & Qt::WindowMinimized))
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarShadeButton, wd);
            down = (tb->activeSubControls & QStyle::SC_TitleBarShadeButton &&
                    (opt->state & QStyle::State_Sunken));
        ...
        if (tb->subControls & QStyle::SC_TitleBarUnshadeButton &&
            tb->titleBarFlags & Qt::WindowShadeButtonHint && tb->titleBarState & Qt::WindowMinimized)
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarUnshadeButton, wd);

            down = tb->activeSubControls & QStyle::SC_TitleBarUnshadeButton &&
                   (opt->state & QStyle::State_Sunken);
        ...
        {
            ir = baseStyle()->proxy()->subControlRect(QStyle::CC_TitleBar, tb,
                                                      QStyle::SC_TitleBarContextHelpButton, wd);

            down = tb->activeSubControls & QStyle::SC_TitleBarContextHelpButton &&
                   (opt->state & QStyle::State_Sunken);
            pm = baseStyle()
                         ->standardIcon(QStyle::SP_TitleBarContextHelpButton, &tool, wd)
                         .pixmap(10, 10);
            tool.rect = ir;
            tool.state = down ? QStyle::State_Sunken : QStyle::State_Raised;
            baseStyle()->proxy()->drawPrimitive(QStyle::PE_PanelButtonTool, &tool, p, wd);
        ...
        if (tb->subControls & QStyle::SC_TitleBarSysMenu && tb->titleBarFlags & Qt::WindowSystemMenuHint)
        {
            ...
            else
            {
                ...
                pm = baseStyle()
                             ->standardIcon(QStyle::SP_TitleBarMenuButton, &tool, wd)
...
bool QdfOfficePaintManager::drawIndicatorToolBarSeparator(const QStyleOption *opt, QPainter *p,
...
// for DockWidget
bool QdfOfficePaintManager::drawIndicatorDockWidgetResizeHandle(const QStyleOption *opt, QPainter *p,
                                                                const QWidget *w) const
{
    QRect rect = opt->rect;
    ...
    if (!(opt->state & QStyle::State_Horizontal) || rect.height() > rect.width() * 4)
    {
        ...
        QdfDrawHelpers::drawGradientFill4(*p, rect, m_clrBarFace, clrSlider, clrSlider, m_clrBarFace);
    }
    else
    {
...
bool QdfOfficePaintManager::drawPanelMenu(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
    ...
    {
        QdfDrawHelpers::draw3dRectEx(*p, opt->rect, d->m_clrMenubarBorder, d->m_clrMenubarBorder);
    }
    return true;
}

bool QdfOfficePaintManager::drawFrameMenu(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    Q_UNUSED(w);
    QDF_D_STYLE(const QdfOfficeStyle)
    QRect rect = opt->rect;
    QdfDrawHelpers::draw3dRectEx(*p, rect, d->m_clrMenubarBorder, d->m_clrMenubarBorder);
    rect.adjust(1, 1, -1, -1);
    QdfDrawHelpers::draw3dRectEx(*p, rect, d->m_clrBackgroundLight, d->m_clrBackgroundLight);
    QRect rectLeft(1, 1, 2, rect.bottom() - 1);
    p->fillRect(rectLeft, m_clrBarHilite);
    return true;
}

bool QdfOfficePaintManager::drawIndicatorMenuCheckMark(const QStyleOption *opt, QPainter *p,
                                                       const QWidget *w) const
{
    Q_UNUSED(w);
    QRect rc = opt->rect;
    ...
    ImageIcons iImage =
            /*isRadio*/ false ? QdfOfficePaintManager::Icon_Radio : QdfOfficePaintManager::Icon_Check;
...
bool QdfOfficePaintManager::drawIndicatorCheckRadioButton(QStyle::PrimitiveElement element,
                                                          ...
                                                          const QWidget *widget) const
{
    ...
    else
    {
        ...
        if (stateChecked == 1)
        {
            ...
            QPen checkPen = QPen(checkMarkColor, QdfDrawHelpers::dpiScaled(1.1));
            ...
            const int checkMarkPadding = QdfDrawHelpers::dpiScaled(3);
            // Draw checkmark
            QPainterPath path;
...
bool QdfOfficePaintManager::drawIndicatorViewItemCheck(QStyle::PrimitiveElement element,
...
bool QdfOfficePaintManager::drawPanelButtonCommand(const QStyleOption *opt, QPainter *p,
                                                   const QWidget *w) const
{
    Q_UNUSED(w);
    QDF_D_STYLE(QdfOfficeStyle)
...
bool QdfOfficePaintManager::drawPanelTipLabel(const QStyleOption *opt, QPainter *p,
                                              const QWidget *w) const
{
    Q_UNUSED(w)
...
bool QdfOfficePaintManager::drawControlEdit(const QStyleOption *opt, QPainter *p,
                                            const QWidget *widget) const
{
    QDF_D_STYLE(QdfOfficeStyle)
...
bool QdfOfficePaintManager::drawFrameLineEdit(const QStyleOption *opt, QPainter *p,
                                              const QWidget *w) const
{
    Q_UNUSED(w);
    QDF_D_STYLE(QdfOfficeStyle)
...
bool QdfOfficePaintManager::drawComboBox(const QStyleOptionComplex *opt, QPainter *p,
                                         const QWidget *w) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
    if (const QStyleOptionComboBox *cmb = qstyleoption_cast<const QStyleOptionComboBox *>(opt))
    {
        ...
        bool dropButtonHot =
                cmb->activeSubControls == QStyle::SC_ComboBoxArrow && (flags & QStyle::State_MouseOver);
        ...
        {
            QRect rcBtn = baseStyle()->proxy()->subControlRect(QStyle::CC_ComboBox, opt,
                                                               QStyle::SC_ComboBoxArrow, w);
            ...
            drawIconByColor(p, QdfOfficePaintManager::Icon_ArowDown, rcBtn, clrFill);
        }
        return true;
    }
    return false;
...
bool QdfOfficePaintManager::drawSpinBox(const QStyleOptionComplex *opt, QPainter *p,
                                        const QWidget *w) const
{
    Q_UNUSED(w);
    QDF_D_STYLE(const QdfOfficeStyle)
    ...
    if (const QStyleOptionSpinBox *sb = qstyleoption_cast<const QStyleOptionSpinBox *>(opt))
    {
        ...
        if (sb->frame && (sb->subControls & QStyle::SC_SpinBoxFrame))
        {
            ...
            p->fillRect(r, enabled && (highlighted || hasFocus)
                                   ? sb->palette.brush(QPalette::Normal, QPalette::Base)
        ...
        QRect rcTop =
                baseStyle()->proxy()->subControlRect(QStyle::CC_SpinBox, &copy, QStyle::SC_SpinBoxUp, w);

        copy = *sb;

        QdfOfficePaintManager::ImageIcons index[2][2] = {
                {   QdfOfficePaintManager::Icon_ArowUp, QdfOfficePaintManager::Icon_ArowDown},
                {QdfOfficePaintManager::Icon_ArowRight, QdfOfficePaintManager::Icon_ArowLeft}
        };
        ...
        if (sb->subControls & QStyle::SC_SpinBoxUp)
        {
            ...
            rcTop = baseStyle()->proxy()->subControlRect(QStyle::CC_SpinBox, sb, QStyle::SC_SpinBoxUp,
                                                         w);
            ...
            else if (sb->activeSubControls == QStyle::SC_SpinBoxUp && (sb->state & QStyle::State_Sunken))
            {
                pressedButton = true;
                     ...
                     (sb->state & QStyle::State_MouseOver))
            {
                hotButton = true;
            }
            ...
            {
                p->fillRect(rcTop.adjusted(0, 1, -1, -1),
                            pressedButton ? m_clrHighlightDn : d->m_clrHighlight);
            }
            drawIcon(p, rcTop, index[false ? 1 : 0][0],
                     enabledIndicator ? QdfOfficePaintManager::Black : QdfOfficePaintManager::Gray,
                     QSize(9, 9));
        }
        ...
        if (sb->subControls & QStyle::SC_SpinBoxDown)
        {
            ...
            QRect rcBottom = baseStyle()->proxy()->subControlRect(QStyle::CC_SpinBox, sb,
                                                                  QStyle::SC_SpinBoxDown, w);
            ...
            else if (sb->activeSubControls == QStyle::SC_SpinBoxDown &&
            ...
            if (pressedButton || hotButton)
            {
                p->fillRect(rcBottom.adjusted(0, 0, -1, -2),
                            pressedButton ? m_clrHighlightDn : d->m_clrHighlight);
            }
...
bool QdfOfficePaintManager::drawToolBoxTabShape(const QStyleOption *, QPainter *, const QWidget *) const
{
    return false;
}

bool QdfOfficePaintManager::drawTabBarTabShape(const QStyleOption *option, QPainter *p,
                                               const QWidget *widget) const
{
    if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option))
    {
        ...
        const int tabOverlap =
                baseStyle()->proxy()->pixelMetric(QStyle::PM_TabBarTabOverlap, option, widget);
...
bool QdfOfficePaintManager::drawIndicatorTabClose(const QStyleOption *opt, QPainter *p,
                                                  const QWidget *w) const
{
    Q_UNUSED(w);
    ...
    ImageIcons iImage = QdfOfficePaintManager::Icon_Close;
...
bool QdfOfficePaintManager::drawShapedFrame(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    bool ret = false;
                ...
                case QFrame::Panel:
                    {
                        if (frameShadow == QFrame::Sunken)
                        {
                            ...
                            if (QTabBar *tabBar = mdiArea->findChild<QTabBar *>())
                            {
                                ...
                                QdfDrawHelpers::draw3dRectEx(*p, rect, m_clrBarDkShadow,
                                                             m_clrBarDkShadow);
                            }
                            ret = true;
                        }
...
bool QdfOfficePaintManager::drawFrameWindow(const QStyleOption *opt, QPainter *p,
                                            const QWidget *wd) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
    if (!wd || !qobject_cast<const QMdiSubWindow *>(wd))
    {
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
        QWidget *widget = (wd && wd->parentWidget()) ? wd->parentWidget() : nullptr;
        if (!qobject_cast<const QWorkspace *>(widget))
        {
            return false;
        }
    ...
    QRect rect = QRect(opt->rect.x(), opt->rect.y() + fwidth, opt->rect.x() + fwidth,
                       opt->rect.height() - fwidth);
    p->fillRect(rect, color);
...
bool QdfOfficePaintManager::drawScrollBar(const QStyleOptionComplex *opt, QPainter *p,
                                          const QWidget *w) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
    ...
    if (const QStyleOptionSlider *scrollbar = qstyleoption_cast<const QStyleOptionSlider *>(opt))
    {
        ...
        {
            newScrollbar.state &= ~QStyle::State_Enabled;
        }
...
bool QdfOfficePaintManager::drawScrollBarLine(QStyle::ControlElement element, const QStyleOption *opt,
                                              QPainter *p, const QWidget *w) const
{
    ...
    if (const QStyleOptionSlider *scrollbar = qstyleoption_cast<const QStyleOptionSlider *>(opt))
    {
        ...
        */
        ...
        QdfOfficePaintManager::ImageIcons index;
        if (opt->state & QStyle::State_Horizontal)
        {
            index = element == QStyle::CE_ScrollBarAddLine ? QdfOfficePaintManager::Icon_ArowRightTab3d
                                                           : QdfOfficePaintManager::Icon_ArowLeftTab3d;
        }
        else
        {
            index = element == QStyle::CE_ScrollBarAddLine ? QdfOfficePaintManager::Icon_ArowDownLarge
                                                           : QdfOfficePaintManager::Icon_ArowUpLarge;
        }

        QdfOfficePaintManager::ImageState state =
                enabled ? stateByColor(clrFill, true) : QdfOfficePaintManager::Gray;
...
bool QdfOfficePaintManager::drawScrollBarSlider(const QStyleOption *opt, QPainter *p,
                                                const QWidget *w) const
{
    Q_UNUSED(w);
...
bool QdfOfficePaintManager::drawScrollBarPage(QStyle::ControlElement element, const QStyleOption *opt,
                                              QPainter *p, const QWidget *w) const
{
    Q_UNUSED(element);
    Q_UNUSED(opt);
...
// for stausBar
bool QdfOfficePaintManager::drawPanelStatusBar(const QStyleOption *opt, QPainter *p,
                                               const QWidget *w) const
{
    Q_UNUSED(w);
...
// for SizeGrip
bool QdfOfficePaintManager::drawSizeGrip(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    Q_UNUSED(w);
    ...
    for (int y = 0; y < 3; y++)
    {
        for (int x = 0; x < 3 - y; x++)
        {
            p->fillRect(QRect(QPoint(pt.x() + 1 - x * 4, pt.y() + 1 - y * 4), QSize(2, 2)),
                        QColor(255, 255, 255));
        }
    }
    return true;
...
// for ViewItem
bool QdfOfficePaintManager::drawPanelItemViewItem(const QStyleOption *opt, QPainter *p,
                                                  const QWidget *w) const
{
    Q_UNUSED(opt);
    Q_UNUSED(p);
...
bool QdfOfficePaintManager::drawHeaderSection(const QStyleOption *opt, QPainter *p,
                                              const QWidget *w) const
{
    Q_UNUSED(w);
...
bool QdfOfficePaintManager::drawHeaderEmptyArea(const QStyleOption *opt, QPainter *p,
                                                const QWidget *w) const
{
    Q_UNUSED(w);
    p->fillRect(opt->rect, m_clrBarFace);
...
bool QdfOfficePaintManager::drawIndicatorHeaderArrow(const QStyleOption *, QPainter *,
                                                     const QWidget *) const
{
    return false;
}

bool QdfOfficePaintManager::drawIndicatorArrow(QStyle::PrimitiveElement pe, const QStyleOption *opt,
                                               QPainter *p, const QWidget *w) const
{
    Q_UNUSED(w);
    switch (pe)
    {
        case QStyle::PE_IndicatorArrowDown:
                ...
                !qobject_cast<const QTabBar *>(w->parentWidget()))
            {
                ...
                ImageIcons iImage = QdfOfficePaintManager::Icon_ArowDown;
...
bool QdfOfficePaintManager::drawGalleryToolButton(const QStyleOption *opt, QPainter *p,
                                                  const QWidget *widget) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
                ...
                qstyleoption_cast<const QStyleOptionToolButton *>(opt))
    {
        if (widget)
        {
            QdfOfficePaintManager::ImageIcons id = QdfOfficePaintManager::Icon_Check;
            if (widget->property(_qdf_PopupButtonGallery).toBool())
            {
                id = QdfOfficePaintManager::Icon_CustomizeArowDown;
            }
            else if (widget->property(_qdf_ScrollUpButtonGallery).toBool())
            {
                id = QdfOfficePaintManager::Icon_ArowUp;
            }
            else if (widget->property(_qdf_ScrollDownButtonGallery).toBool())
            {
                id = QdfOfficePaintManager::Icon_ArowDown;
            }

            if (id != QdfOfficePaintManager::Icon_Check)
            {
                bool enabled = opt->state & QStyle::State_Enabled;
                bool highlighted = opt->state & QStyle::State_MouseOver;
                bool pressed = opt->state & QStyle::State_Sunken;
                ...
                drawIcon(p, rect, id, QdfOfficePaintManager::White);
                drawIcon(p, rect, id,
                         enabled ? QdfOfficePaintManager::Black : QdfOfficePaintManager::Gray);
...
void QdfOfficePaintManager::drawMenuItemSeparator(const QStyleOption *opt, QPainter *p,
                                                  const QWidget *widget) const
{
    QDF_D_STYLE(const QdfOfficeStyle)
    if (const QStyleOptionMenuItem *menuitem = qstyleoption_cast<const QStyleOptionMenuItem *>(opt))
    {
        if (menuitem->text.isEmpty())
        {
            ...
            const int iconSize =
                    baseStyle()->proxy()->pixelMetric(QStyle::PM_ToolBarIconSize, opt, widget);
            int yoff = (y - 1 + h / 2);
            int xIcon = iconSize;
        ...
        else
        {
            ...
            font.setBold(true);
            p->setFont(font);
            p->setPen(opt->state & QStyle::State_Enabled ? d->m_clrMenuPopupText
...
void QdfOfficePaintManager::drawRectangle(QPainter *p, const QRect &rect, bool selected, bool pressed,
                                          ...
                                          BarPosition barPos) const
{
    ...
    else if (barPos == BarPopup && selected && barType == TypePopup)
    {
        ...
        QdfDrawHelpers::draw3dRectEx(*p, rc, m_clrMenuItemBorder, m_clrMenuItemBorder);
    }
...
void QdfOfficePaintManager::setupPalette(QWidget *widget) const
{
    QDF_D_STYLE(QdfOfficeStyle)
...
bool QdfOfficePaintManager::pixelMetric(QStyle::PixelMetric pm, const QStyleOption *opt,
                                        const QWidget *w, int &ret) const
{
    Q_UNUSED(opt);
    Q_UNUSED(w);
    switch (pm)
    {
        case QStyle::PM_MenuButtonIndicator:
            if (qobject_cast<const QToolButton *>(w))
            {
                ret = int(QdfDrawHelpers::dpiScaled(qreal(iImageWidth + 1)));
                return true;
            }
            break;
...
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
            {
                QWidget *widget = (w && w->parentWidget()) ? w->parentWidget() : nullptr;
                if (qobject_cast<const QWorkspace *>(widget))
                {
                    ret = 3;
                    return true;
...
bool QdfOfficePaintManager::styleHint(QStyle::StyleHint hint, const QStyleOption *opt, const QWidget *w,
...
void QdfOfficePaintManager::modifyColors()
{
    QDF_D_STYLE(QdfOfficeStyle)
    m_clrAccentLight = QdfDrawHelpers::colorMakePale(d->m_clrAccent.rgb(), .9);
    m_clrAccentHilight = QdfDrawHelpers::pixelAlpha(d->m_clrAccent.rgb(), 115);
    m_clrAccentText = QdfDrawHelpers::pixelAlpha(d->m_clrAccent.rgb(), 75);

    d->m_clrFrameBorderActive0 = m_clrAccentText;
    d->m_clrFrameBorderInactive0 = QdfDrawHelpers::colorMakeLighter(m_clrBarDkShadow.rgb());
    d->m_clrTabSelectedText = m_clrAccentText;

    d->m_clrHighlight = m_clrAccentLight;
    d->m_clrHighlightNC = d->m_clrHighlight;
    m_clrHighlightDn = QdfDrawHelpers::colorMakeDarker(m_clrAccentLight.rgb(), .14);
    m_clrHighlightDnNC = m_clrHighlightDn;
    m_clrHighlightChecked = QdfDrawHelpers::colorMakeDarker(m_clrAccentLight.rgb(), .05);
    ...
    d->m_clrMenubarBorder = QdfDrawHelpers::colorMakeLighter(m_clrBarDkShadow.rgb());// m_clrMenuBorder
...
QdfOfficePaintManager::ImageState QdfOfficePaintManager::stateByColor(const QColor &color,
                                                                      bool backgroundColor) const
{
    if (!color.isValid())
    {
        return Black;
...
void QdfOfficePaintManager::drawIcon(QPainter *p, const QRect &rectImage, ImageIcons index,
                                     ImageState state, const QSize &sizeImageDest) const
{
...
void QdfOfficePaintManager::drawIcon(QPainter *p, const QPoint &pos, ImageIcons index, ImageState state,
...
QPixmap QdfOfficePaintManager::mapTo3dColors(const QPixmap &px, const QColor &clrSrc,
                                             const QColor &clrDest) const
{
...
void QdfOfficePaintManager::loadPixmap()
{
