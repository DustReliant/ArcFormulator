#ifndef DESKTOPFRAMEWORK_QDFOFFICESTYLE_P_H
#define DESKTOPFRAMEWORK_QDFOFFICESTYLE_P_H
...
public:
    ...
    // for Buttons
    virtual bool drawIndicatorCheckRadioButton(QStyle::PrimitiveElement element, const QStyleOption *,
                                               QPainter *, const QWidget *) const = 0;
...
public:
    virtual bool pixelMetric(QStyle::PixelMetric pm, const QStyleOption *opt, const QWidget *w,
                             int &ret) const = 0;
...
Q_DECLARE_INTERFACE(Qdf::OfficePaintManagerInterface, "canpool.Qdf.OfficePaintManagerInterface")

QDF_BEGIN_NAMESPACE

class QdfOfficeStylePrivate : public QdfCommonStylePrivate
{
public:
    QDF_DECLARE_EX_PUBLIC(QdfOfficeStyle)
public:
    explicit QdfOfficeStylePrivate();
    virtual ~QdfOfficeStylePrivate();
...
public:
    ...
    QColor accentIndexToColor(QdfOfficeStyle::AccentColor accentcolor) const;
    bool isScrollBarsIgnored() const;

public:
    QColor m_clrAccent;
    QdfOfficeStyle::AccentColor m_curAccentColor;
    ...
    QdfOfficeStyle::OfficePopupDecoration m_typePopupStyle;

    QStyle *m_popupProxy;
    uint m_refCountProxy;

    QdfOfficeStyle::Theme m_themeType;
    QHash<const QWidget *, QBrush> m_customBrushWidgets;// widgets whose brush we tampered
};

class QdfOfficePaintManager : public QdfCommonPaintManager, public OfficePaintManagerInterface
{
    Q_OBJECT
    Q_INTERFACES(Qdf::OfficePaintManagerInterface)
public:
    explicit QdfOfficePaintManager(QdfCommonStyle *baseStyle);
    virtual ~QdfOfficePaintManager();
...
public:
    QdfOfficeStyle::Theme theme() const;
    QString theme(const QString &str) const;
    QColor textColor(bool selected, bool pressed, bool enabled, bool checked, bool popuped,
...
public:
    ...
    virtual bool isTopLevelMenuItemUpperCase(const QWidget *) const
    {
        return false;
    }
...
protected:
    ...
    void drawIcon(QPainter *p, const QPoint &pos, ImageIcons index, ImageState state,
...
private:
    Q_DISABLE_COPY(QdfOfficePaintManager)
};

QDF_END_NAMESPACE

#endif//DESKTOPFRAMEWORK_QDFOFFICESTYLE_P_H