...
QRect QdfPopupDrawHelper::subControlRect(ComplexControl control, const QStyleOptionComplex *opt,
                                         SubControl subControl, const QWidget *widget) const
{
    ...
    {
        case CC_TitleBar:
            if (const QdfStyleOptionPopupTitleBar *bar =
                        qstyleoption_cast<const QdfStyleOptionPopupTitleBar *>(opt))
            {
                    ...
                    case SC_TitleBarLabel:
                        {
                            ...
                            {
                                QRect ir = proxy()->subControlRect(CC_TitleBar, bar,
                                                                   SC_TitleBarCloseButton, widget);
                                labelRect.adjust(bar->subControls & SC_TitleBarSysMenu ? delta
                                                                                       : controlMargin,
                                                 0, -ir.width(), controlMargin);
                            }
                            else
                            {
                                labelRect.adjust(bar->subControls & SC_TitleBarSysMenu ? delta
                                                                                       : controlMargin,
                                                 0, controlMargin, controlMargin);
                            }
                            rect = labelRect;
                            break;
                        }
...
int QdfPopupDrawHelper::pixelMetric(PixelMetric metric, const QStyleOption *option,
...
QStyle::SubControl QdfPopupDrawHelper::hitTestComplexControl(ComplexControl control,
                                                             ...
                                                             const QWidget *widget) const
{
    ...
    switch (control)
    {
        case CC_TitleBar:
            {
                const QdfStyleOptionPopupTitleBar *bar =
                        qstyleoption_cast<const QdfStyleOptionPopupTitleBar *>(opt);
...
void QdfPopupDrawHelper::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
                                            QPainter *p, const QWidget *widget) const
{
    switch (cc)
    {
        case CC_TitleBar:
            if (const QdfStyleOptionPopupTitleBar *tb =
                        qstyleoption_cast<const QdfStyleOptionPopupTitleBar *>(opt))
            {
                QRect rect = tb->rect;

                if (tb->subControls &
                    SC_TitleBarSysMenu /* && tb->titleBarFlags & Qt::WindowSystemMenuHint*/)
                {
                ...
                p->drawText(labelRect, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextSingleLine,
                            p->fontMetrics().elidedText(tb->text, Qt::ElideRight,
                                                        qMin(rect.width(), labelRect.width() - 2)));

                p->setFont(oldFont);

                if (tb->subControls & SC_TitleBarCloseButton)
                {
                    bool down = tb->activeSubControls & SC_TitleBarCloseButton &&
                    ...
                    if (!tb->pixmapCloseButton.isNull())
                    {
                        ...
                        else if (tb->activeSubControls & SC_TitleBarCloseButton &&
                                 (opt->state & State_MouseOver))
                        {
                            state = 1;
                        }
                        QRect rcImage = ::getSource(tb->pixmapCloseButton.rect(), state, 3);
                        pm = tb->pixmapCloseButton.copy(rcImage.left(), rcImage.top(), rcImage.width(),
                                                        rcImage.height());
                    }
                    else
                    {
    ...
    switch (element)
    {
        case PE_FrameWindow:
            if (const QStyleOptionFrame *optFrame = qstyleoption_cast<const QStyleOptionFrame *>(option))
            {
                ...
                rect.adjust(optFrame->lineWidth, optFrame->lineWidth, -optFrame->lineWidth,
                            -optFrame->lineWidth);
                rect.setBottom(rect.top() + 30);
                QdfDrawHelpers::drawGradientFill(
                        *p, rect,
...
*/
int PopupOffice2000DrawHelper::pixelMetric(PixelMetric metric, const QStyleOption *option,
                                           const QWidget *widget) const
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
void PopupOffice2000DrawHelper::drawPrimitive(PrimitiveElement element, const QStyleOption *option,
                                              QPainter *p, const QWidget *widget) const
{
    switch (element)
    {
        case PE_FrameWindow:
            if (const QStyleOptionFrame *optFrame = qstyleoption_cast<const QStyleOptionFrame *>(option))
            {
                ...
                QdfDrawHelpers::draw3DRect(*p, m_palFrame.shadow().color(), m_palFrame.shadow().color(),
                ...
                QdfDrawHelpers::drawGradientFill(*p, rcBackground, QColor(0, 0, 128),
                                                 QColor(24, 180, 192), false);
                ...
                QdfDrawHelpers::draw3DRect(*p, m_palFrame.shadow().color(), m_palFrame.shadow().color(),
                                           rcBackground.x(), rcBackground.y(), rcBackground.width() - 1,
                                           rcBackground.height() - 1, false);

                rcBackground.adjust(1, 1, -1, -1);
                rcBackground.setRight(rcBackground.left() + 30);
                QdfDrawHelpers::drawGradientFill(*p, rcBackground, QColor(0, 0, 128), QColor(0, 0, 128),
                                                 true);
...
QRect PopupOffice2003DrawHelper::subControlRect(ComplexControl control,
                                                ...
                                                const QWidget *widget) const
{
    ...
    switch (control)
    {
        case CC_TitleBar:
            if (const QdfStyleOptionPopupTitleBar *bar =
                        qstyleoption_cast<const QdfStyleOptionPopupTitleBar *>(option))
            {
                switch (subControl)
                {
                    case SC_TitleBarLabel:
                    ...
                    case SC_TitleBarCloseButton:
                        {
                            ...
                            int offsetX = rc.right() - 13 - 2;
                            int offsetY =
                                    titleBarHeight + lineWidth * 2;//(rc.height() - rcButton.height())/2;
                            rcButton.translate(offsetX, offsetY);
                            rect = rcButton;
                            break;
                        }
...
int PopupOffice2003DrawHelper::pixelMetric(PixelMetric metric, const QStyleOption *option,
                                           const QWidget *widget) const
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
void PopupOffice2003DrawHelper::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
                                                   QPainter *p, const QWidget *widget) const
{
    switch (cc)
    {
        case CC_TitleBar:
                        ...
                        qstyleoption_cast<const QdfStyleOptionPopupTitleBar *>(opt))
            {
                ...
                if (tb->subControls & SC_TitleBarCloseButton)
                {
                    bool down = tb->activeSubControls & SC_TitleBarCloseButton &&
                                (opt->state & State_Sunken);
                    bool select = tb->activeSubControls & SC_TitleBarCloseButton &&
                                  (opt->state & State_MouseOver);
        ...
        case CC_ToolButton:
                        ...
                        qstyleoption_cast<const QStyleOptionToolButton *>(opt))
            {
                ...
                else if (toolbutton->features & QStyleOptionToolButton::HasMenu)
                {
                    ...
                    newBtn.rect = QRect(ir.right() + 5 - mbi, ir.y() + ir.height() - mbi + 4, mbi - 6,
                                        mbi - 6);
...
void PopupOffice2003DrawHelper::drawPrimitive(PrimitiveElement element, const QStyleOption *option,
                                              QPainter *p, const QWidget *widget) const
{
    switch (element)
    {
        case PE_PanelButtonTool:
            {
                ...
                {
                    QPen savePen = p->pen();
                    p->setPen(option->state & State_Sunken ? m_clrButtonPressedBorder
                                                           : m_clrButtonSelectedBorder);
        ...
        case PE_FrameWindow:
            if (const QStyleOptionFrame *optFrame = qstyleoption_cast<const QStyleOptionFrame *>(option))
            {
                ...
                QdfDrawHelpers::draw3DRect(*p, m_palFrame.color(QPalette::Light),
                                           m_palFrame.color(QPalette::Dark), rect.x(), rect.y(),
...
{
    m_helper = new QdfStyleHelper(this);
}
    ...
    switch (element)
    {
        case PE_FrameWindow:
            if (const QStyleOptionFrame *optFrame = qstyleoption_cast<const QStyleOptionFrame *>(option))
            {
                ...
                QdfDrawHelpers::draw3DRect(*p, QColor(166, 180, 207), QColor(69, 86, 144), rect.x(),
                                           rect.y(), rect.width() - 1, rect.height() - 1, true);
                rect.adjust(1, 1, -1, -1);
                QdfDrawHelpers::draw3DRect(*p, QColor(255, 255, 255), QColor(207, 222, 244), rect.x(),
                                           rect.y(), rect.width() - 1, rect.height() - 1, true);
                rect.adjust(1, 1, -1, -1);
                ...
                QdfDrawHelpers::drawGradientFill(*p, rcBackground, QColor(207, 215, 236),
                                                 QColor(255, 255, 255), true);

                rcBackground.setHeight(titleBarHeight + 1);
                QdfDrawHelpers::drawGradientFill(*p, rcBackground, QColor(255, 255, 255),
                                                 QColor(207, 221, 244), true);

                rcBackground.setHeight(40);
                QdfDrawHelpers::drawGradientFill(*p, rcBackground, QColor(207, 221, 244),
                                                 QColor(255, 255, 255), true);
                ...
                QRect rcFrame =
                        rect;//(rcClient.left, rcClient.top + 22, rcClient.right, rcClient.bottom);
                rcFrame.setTop(rect.top() + titleBarHeight);
                QdfDrawHelpers::draw3DRect(*p, QColor(114, 142, 184), QColor(185, 201, 239), rcFrame.x(),
                                           rcFrame.y(), rcFrame.width() - 1, rcFrame.height() - 1, true);
                break;
            }
            // fallthrough
        default:
