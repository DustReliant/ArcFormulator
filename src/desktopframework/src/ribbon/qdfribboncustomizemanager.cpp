...
QString QdfCustomizeEngineAbstract::categoryAllActions() const { return m_allCategoryName; }

QList<QAction *> QdfCustomizeEngineAbstract::actionsAll() const { return m_allNameToActions.values(); }
...
QStringList QdfCustomizeEngineAbstract::categories() const { return m_lstCategory; }

QString QdfCustomizeEngineAbstract::actionId(QAction *action) const { return m_allActionsToName.value(action); }
...
QdfToolBarCustomizeEngine::QdfToolBarCustomizeEngine(QObject *parent) : QdfCustomizeEngineAbstract(parent) {}
...
QMap<QToolBar *, QList<QAction *>> QdfToolBarCustomizeEngine::defaultToolBars() const { return m_defaultToolBars; }
...
void QdfToolBarCustomizeEngine::addDefaultToolBar(QToolBar *toolBar)
{
    ...
    while (itAction.hasNext())
    {
        QAction *action = itAction.next();
        if (action->isVisible() &&
            action->property(__qdf_Action_Invisible).toString() != QLatin1String("__qdf_Action_Invisible") &&
            action->property(__qdf_Quick_Access_Button).toString() != QLatin1String("__qdf_Quick_Access_Button"))
        {
            setActionId(action);
...
void QdfToolBarCustomizeEngine::saveStateQuickAccessBar(QXmlStreamWriter &stream) const
{
    ...
    while (itToolBar != m_defaultToolBars.constEnd())
    {
        ...
        {
            qWarning("QdfToolBarCustomizeEngine::saveStateQuickAccessBar(): 'objectName' not set for QToolBar "
        ...
        while (itAction.hasNext())
        {
            ...
            if (QAction *action = itAction.next())
            {
                ...
                if (id.isEmpty())
                {
                    qWarning("QdfToolBarCustomizeEngine::saveStateQuickAccessBar(): 'objectName' not set for QAction "
...
void QdfToolBarCustomizeEngine::saveState(QXmlStreamWriter &stream) const { saveStateQuickAccessBar(stream); }

bool QdfToolBarCustomizeEngine::restoreState(QXmlStreamReader &stream) { return restoreStateQuickAccessBar(stream); }
...
QString QdfRibbonBarCustomizeEngine::pageId(QdfRibbonPage *page) const { return m_allPagesToNameId.value(page); }
...
QString QdfRibbonBarCustomizeEngine::groupId(QdfRibbonGroup *group) const { return m_allGroupsToNameId.value(group); }
...
QString QdfRibbonBarCustomizeEngine::pageDefaultName(QdfRibbonPage *page) const { return m_defaultPagesName.value(page); }
...
QList<QdfRibbonPage *> QdfRibbonBarCustomizeEngine::defaultRibbonBar() const { return m_defaultPages; }

QMap<QdfRibbonGroup *, QString> QdfRibbonBarCustomizeEngine::defaultGroupsName() const { return m_defaultNameGroups; }
...
void QdfRibbonBarCustomizeEngine::setRibbonBar()
{
    ...
    else
    {
        ...
        while (itRegularPage.hasNext())
        {
            ...
            if (!m_regularPages.contains(currentPage))
            {
                if (currentPage->property(__qdf_Widget_Custom).toString() == QLatin1String("__qdf_Page_Custom"))
                {
...
{
    for (QMap<QdfRibbonPage *, QList<QdfRibbonGroup *>>::ConstIterator itCurPages = m_regularPageGroups.begin();
         itCurPages != m_regularPageGroups.end(); ++itCurPages)
    {
        ...
        if (groupsPage != realGroups || m_regularNameGroups.size() > 0 || m_regularGroupsActions.size() > 0)
        {
            ...
            while (itNewGroups.hasNext())
            {
                ...
                if (group->property(__qdf_Widget_Custom).toString() == QLatin1String("__qdf_Group_Custom"))
                {
                    group->clear();
...
void QdfRibbonBarCustomizeEngine::updateRibbonBar()
{
    ...
    for (QList<QdfRibbonPage *>::const_iterator it = listPage.constBegin(); it != listPage.constBegin(); ++it)
    {
...
{
    for (QMap<QdfRibbonPage *, QList<QdfRibbonGroup *>>::ConstIterator itPages = m_regularPageGroups.begin();
...
void QdfRibbonBarCustomizeEngine::saveStateGroup(QXmlStreamWriter &stream, QdfRibbonPage *page) const
{
    ...
    while (itGroup.hasNext())
    {
        if (QdfRibbonGroup *group = itGroup.next())
        {
            ...
            bool custom = group->property(__qdf_Widget_Custom).toString() == QLatin1String("__qdf_Group_Custom");
            bool copy = group->property(__qdf_Widget_Copy).toString() == QLatin1String("__qdf_Group_Copy");
            ...
            if (nameId.isEmpty())
            {
                if (!(custom || copy))
                {
                    qWarning("QdfRibbonBarCustomizeEngine::saveStateGroup(): 'objectName' not set for QdfRibbonGroup "
                             "%p '%s', using 'text' instead",
...
bool QdfRibbonBarCustomizeEngine::restoreStateGroup(QXmlStreamReader &stream, QdfRibbonPage *page)
{
    ...
    while (stream.tokenType() == QXmlStreamReader::StartElement)
    {
        ...
        else if (!(customGroup || copyGroup))
        {
            ...
            for (QList<QdfRibbonGroup *>::iterator it = listGroup.begin(); it != listGroup.end() && group == nullptr;
                 ++it)
            {
    ...
    while (itPage.hasNext())
    {
        if (QdfRibbonPage *page = itPage.next())
        {
            ...
            if (nameId.isEmpty())
            {
                ...
                {
                    qWarning("QdfRibbonBarCustomizeEngine::saveState(): 'pageId' not set for QdfRibbonPage "
                             "%p '%s', using 'text' instead",
            ...
            stream.writeAttribute(QDF_DIC_ATTRNAME_RIBBON_VISIBLE, QVariant(page->isVisible()).toString());
...
bool QdfRibbonBarCustomizeEngine::restoreStatePage(QXmlStreamReader &stream)
{
    ...
    while (stream.tokenType() == QXmlStreamReader::StartElement)
    {
        ...
        else if (!customPage)
        {
            ...
            for (QList<QdfRibbonPage *>::iterator it = listPage.begin(); it != listPage.end() && page == nullptr; ++it)
            {
                if ((*it)->title() == nameId || (*it)->title() == title)
                {
                    page = (*it);
...
QdfRibbonCustomizeManager::~QdfRibbonCustomizeManager() { QDF_FINI_PRIVATE(); }
...
void QdfRibbonCustomizeManager::setEditMode(bool set)
{
    ...
    if (d->m_editMode)
    {
             ...
             it != d->m_ribbonManager->m_regularPages.end(); ++it)
        {
            ...
            for (QList<QdfRibbonGroup *>::iterator iGroup = listGrops.begin(); iGroup != listGrops.end(); ++iGroup)
            {
...
void QdfRibbonCustomizeManager::reset(QToolBar *toolBar)
{
    ...
    if (toolBar == nullptr)
    {
        ...
        for (QMap<QdfRibbonPage *, QList<QdfRibbonGroup *>>::iterator it = d->m_ribbonManager->m_regularPageGroups.begin();
...
QdfRibbonGroup *QdfRibbonCustomizeManager::createGroup(QdfRibbonPage *page, const QString &groupName, int index)
{
    QDF_D(QdfRibbonCustomizeManager)
    if (!isEditMode())
    {
...
bool QdfRibbonCustomizeManager::isPageVisible(QdfRibbonPage *page) const { return !isPageHidden(page); }

void QdfRibbonCustomizeManager::setPageVisible(QdfRibbonPage *page, bool visible) { setPageHidden(page, !visible); }
