...
bool QdfRibbonGalleryPrivate::isItemChecked(QdfRibbonGalleryItem *pItem) const { return m_checkedItem == pItem; }
...
{
    return m_scrollBar ? m_scrollBar->sizeHint().width() : m_buttonScrollUp ? m_buttonScrollUp->width()
                                                                            : 0;
}
...
QdfRibbonGalleryItem::~QdfRibbonGalleryItem() { QDF_FINI_PRIVATE(); }
...
void QdfRibbonGalleryItem::setSizeHint(const QSize &size) { setData(Qt::SizeHintRole, size); }

QIcon QdfRibbonGalleryItem::icon() const { return qvariant_cast<QIcon>(data(Qt::DecorationRole)); }

void QdfRibbonGalleryItem::setIcon(const QIcon &icon) { setData(Qt::DecorationRole, icon); }

QString QdfRibbonGalleryItem::caption() const { return data(Qt::DisplayRole).toString(); }

void QdfRibbonGalleryItem::setCaption(const QString &caption) { setData(Qt::DisplayRole, caption); }

QString QdfRibbonGalleryItem::toolTip() const { return data(Qt::ToolTipRole).toString(); }

void QdfRibbonGalleryItem::setToolTip(const QString &toolTip) { setData(Qt::ToolTipRole, toolTip); }

QString QdfRibbonGalleryItem::statusTip() const { return data(Qt::StatusTipRole).toString(); }

void QdfRibbonGalleryItem::setStatusTip(const QString &statusTip) { setData(Qt::StatusTipRole, statusTip); }
...
void QdfRibbonGalleryItem::draw(QPainter *p, QdfRibbonGallery *gallery, QRect rectItem, bool enabled, bool selected,
...
QdfRibbonGalleryItem *QdfRibbonGalleryGroup::addItemFromMap(const QString &caption, int mapIndex, const QPixmap &map,
...
void QdfRibbonGalleryGroup::appendItem(QdfRibbonGalleryItem *item) { insertItem(itemCount(), item); }
...
QdfRibbonGallery::~QdfRibbonGallery() { QDF_FINI_PRIVATE(); }
...
bool QdfRibbonGallery::isShowAsButton() const { return false; }
...
QSize QdfRibbonGallery::minimumSizeHint() const
{
    ...
    QRect rcBorders(borders());
    int width = qMax(16, galleryGroup()->qdf_d()->m_sizeItem.width()) + rcBorders.left() + rcBorders.right();
    ...
    int height = qMax(galleryGroup()->qdf_d()->m_sizeItem.height() + rcBorders.top() + rcBorders.bottom(), 32);
...
void QdfRibbonGallery::focusOutEvent(QFocusEvent *event) { QWidget::focusOutEvent(event); }
