...
bool QdfRibbonPagePrivate::collapseGroups(int &leftOffset, int actualWidth, QdfRibbonControlSizeDefinition::GroupSize size,
                                          bool adjust)
{
...
bool QdfRibbonPagePrivate::expandGroups(int &leftOffset, int actualWidth, QdfRibbonControlSizeDefinition::GroupSize size,
                                        bool adjust)
{
    Q_ASSERT(m_layoutDirection == Qt::RightToLeft || m_layoutDirection == Qt::LeftToRight);
...
QdfRibbonControlSizeDefinition::GroupSize QdfRibbonPagePrivate::getMinGroupSize() const
{
    ...
    for (QList<QdfRibbonGroup *>::const_iterator it = m_listGroups.constBegin(); it != m_listGroups.constEnd(); ++it)
    {
...
static QdfRibbonControlSizeDefinition::GroupSize qtc_size_inc(QdfRibbonControlSizeDefinition::GroupSize &size)
{
...
static QdfRibbonControlSizeDefinition::GroupSize qtc_size_dec(QdfRibbonControlSizeDefinition::GroupSize &size)
{
    QdfRibbonControlSizeDefinition::GroupSize ret = size;
    size = (QdfRibbonControlSizeDefinition::GroupSize)(size - 1);
    return ret;
...
void QdfRibbonPage::setVisible(bool visible)
{
    ...
    else
    {
        ...
        if (QdfRibbonBar *ribbonBar = qobject_cast<QdfRibbonBar *>(parentWidget()))
        {
            ...
            if (d->m_associativeTab && d->m_contextColor != QdfRibbonPage::ContextColorNone && saveVisible != visible)
            {
#ifdef Q_OS_WIN
                if (QdfRibbonBar *ribbonBar = qobject_cast<QdfRibbonBar *>(parentWidget()))
                {
                ...
                if (!rect().contains(mouseEvent->pos()))
                {
                    ...
                    if (QApplication::activePopupWidget() == this)
                    {
                        if (QWidget *clickedTab =
                                    qobject_cast<QdfRibbonTab *>(QApplication::widgetAt(mouseEvent->globalPos())))
                        {
...
void QdfRibbonPage::paintEvent(QPaintEvent *event)
{
    ...
    if ((windowFlags() & Qt::Popup))
    {
        ...
        style()->drawControl((QStyle::ControlElement) QdfRibbonStyle::CE_RibbonGroups, &opt, &p, this->parentWidget());
    }
}
