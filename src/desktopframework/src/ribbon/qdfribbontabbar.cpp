...
QdfContextHeader::~QdfContextHeader() { firstTab->setContextHeader(nullptr); }
...
QSize QdfRibbonTabPrivate::sizeForWidth(int w) const
{
    ...
    bool itemUpperCase = (bool) q->style()->styleHint((QStyle::StyleHint) QdfRibbonStyle::SH_RibbonItemUpperCase, 0, q);
    if (!text.isEmpty())
    {
        if (itemUpperCase)
        {
...
QRectF QdfRibbonTabPrivate::layoutRect() const { return documentRect(); }
...
void QdfRibbonTab::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect().intersects(opt.rect))
    {
        ...
        style()->drawControl((QStyle::ControlElement) QdfRibbonStyle::CE_RibbonTabShapeLabel, &opt, &p, this);
    }
}
...
void QdfRibbonTab::mousePressEvent(QMouseEvent *event)
{
    ...
    if (QdfRibbonBar *rb = qobject_cast<QdfRibbonBar *>(parentWidget()->parentWidget()))
    {
        if (rb->isBackstageVisible())
        {
            ...
            {
                if (QdfRibbonBackstageView *backstage = qobject_cast<QdfRibbonBackstageView *>(button->backstage()))
                {
                    backstage->close();
                }
            }
...
bool QdfRibbonTabBarPrivate::validWidth() const { return m_validRect; }
...
/* QdfRibbonTabBar */
QdfRibbonTabBar::QdfRibbonTabBar(QWidget *pParent) : QWidget(pParent) { QDF_INIT_PRIVATE(QdfRibbonTabBar); }

QdfRibbonTabBar::~QdfRibbonTabBar() { QDF_FINI_PRIVATE(); }

int QdfRibbonTabBar::addTab(const QString &text) { return insertTab(-1, text); }

int QdfRibbonTabBar::addTab(const QIcon &icon, const QString &text) { return insertTab(-1, icon, text); }

int QdfRibbonTabBar::insertTab(int index, const QString &text) { return insertTab(index, QIcon(), text); }
...
QAction *QdfRibbonTabBar::addAction(const QIcon &icon, const QString &text, Qt::ToolButtonStyle style, QMenu *menu)
{
