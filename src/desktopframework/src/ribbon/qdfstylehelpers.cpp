...
void QdfDrawHelpers::draw3DRect(QPainter &painter, const QColor &colLight, const QColor &colDark, int x, int y, int w, int h,
                                bool up)
{
    QPen savePen = painter.pen();
...
void QdfDrawHelpers::draw3dRectEx(QPainter &painter, const QRect &rect, const QColor &clrTopLeft, const QColor &clrBottomRight)
{
    QdfDrawHelpers::draw3dRectEx(painter, rect.x(), rect.y(), rect.width(), rect.height(), clrTopLeft, clrBottomRight);
}

void QdfDrawHelpers::draw3dRectEx(QPainter &painter, int x, int y, int cx, int cy, const QColor &clrTopLeft,
...
void QdfDrawHelpers::drawRectangle(QPainter &painter, const QRect &rect, const QColor &pen, const QColor &brush)
{
    if (brush.isValid())
    {
...
void QdfDrawHelpers::drawTriangle(QPainter &painter, const QPoint &pt0, const QPoint &pt1, const QPoint &pt2,
                                  const QColor &color)
{
...
void QdfDrawHelpers::drawGradientFill(QPainter &painter, const QRect &rect, const QColor &crFrom, const QColor &crTo,
                                      bool horz)
{
    painter.save();
...
void QdfDrawHelpers::drawGradientFill4(QPainter &painter, const QRect &rect, const QColor &crFrom1, const QColor &crTo1,
...
double QdfDrawHelpers::MakeHue_TWO(double H) { return MakeHue_ONE(H / 360.0) * 360.0; }
...
{
    QRgb clrFinal = qRgb(qMin(255, (qRed(srcPixel) * percent) / 100), qMin(255, (qGreen(srcPixel) * percent) / 100),
                         qMin(255, (qBlue(srcPixel) * percent) / 100));
    return clrFinal;
}

QLinearGradient QdfDrawHelpers::generateGradient(const QRect &rect, const QColor &crFrom, const QColor &crTo, bool bHorz)
{
...
QImage QdfDrawHelpers::addaptColors(const QImage &imageSrc, QRgb clrBase, QRgb clrTone, bool clampHue /*= true*/)
{
...
int QdfDrawHelpers::getDPI() { return defaultDpiX(); }

int QdfDrawHelpers::getDPIToPercent() { return getDPI() * 100 / logPixel; }
...
QIcon QdfDrawHelpers::createIconStyleWidget(QWidget *widget)
{
    if (qobject_cast<QCheckBox *>(widget) || qobject_cast<QRadioButton *>(widget))
    {
        ...
        qApp->style()->drawPrimitive(isRadio ? QStyle::PE_IndicatorRadioButton : QStyle::PE_IndicatorCheckBox, &opt, &p,
                                     0);
...
QdfStyleOptionFrame::QdfStyleOptionFrame(const QdfStyleOptionFrame &other) : QStyleOption(Version, Type) { *this = other; }
...
QdfStyleOptionRibbonBackstage::QdfStyleOptionRibbonBackstage() { menuWidth = 132; }
...
QdfStyleOptionRibbon::QdfStyleOptionRibbon(const QdfStyleOptionRibbon &other) : QStyleOption(Version, Type) { *this = other; }
...
QdfStyleOptionTitleBar::QdfStyleOptionTitleBar(const QdfStyleOptionTitleBar &other) : QStyleOptionTitleBar(other)
{
    *this = other;
}
...
QdfStyleCaptionButton::QdfStyleCaptionButton(const QdfStyleCaptionButton &other) : QStyleOptionToolButton(other)
{
    *this = other;
}
...
QdfStyleSystemToolButton::QdfStyleSystemToolButton(const QdfStyleSystemToolButton &other) : QStyleOptionToolButton(other)
{
    *this = other;
}


QdfStyleHintReturnThemeColor::QdfStyleHintReturnThemeColor(const QString &secName, const QString &kName, QColor col)
    : QStyleHintReturn()
{
    sectionName = secName;
    keyName = kName;
...
QdfStyleRibbonQuickAccessBar::QdfStyleRibbonQuickAccessBar() { quickAccessBarPosition = QdfRibbonBar::QATopPosition; }

QdfStyleRibbonQuickAccessBar::QdfStyleRibbonQuickAccessBar(const QdfStyleRibbonQuickAccessBar &other) : QStyleOption(other)
{
    *this = other;
}
...
QdfStyleRibbonOptionHeader::QdfStyleRibbonOptionHeader(const QdfStyleRibbonOptionHeader &other) : QStyleOptionHeader(other)
{
    *this = other;
