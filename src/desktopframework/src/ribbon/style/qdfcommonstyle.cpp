...
QdfStyleHelper::~QdfStyleHelper() { delete m_pSettings; }
...
QdfCommonAnimation::~QdfCommonAnimation() { setWidget(nullptr); }
        ...
        case 32:
            {
                ...
                for (int sy = 0; sy < sh; sy++)
                {
                    ...
                    for (int sx = 0; sx < sw; sx++)
                    {
                        quint32 bp = back[sx];
                        quint32 fp = front[sx];
                        mixed[sx] = qRgba((qRed(bp) * ia + qRed(fp) * a) >> 8, (qGreen(bp) * ia + qGreen(fp) * a) >> 8,
                                          (qBlue(bp) * ia + qBlue(fp) * a) >> 8, (qAlpha(bp) * ia + qAlpha(fp) * a) >> 8);
                    }
                    mixed_data += bpl;
                    back_data += bpl;
                    front_data += bpl;
...
QdfCommonStyle *QdfCommonPaintManager::baseStyle() const { return m_base; }

QdfStyleHelper &QdfCommonPaintManager::helper() const { return baseStyle()->helper(); }

QPixmap QdfCommonPaintManager::cached(const QString &img) const { return baseStyle()->cached(img); }

QPixmap QdfCommonPaintManager::cachedPath(const QString &img) const { return baseStyle()->QdfCommonStyle::cached(img); }
...
void QdfCommonPaintManager::drawImage(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
...
void QdfCommonPaintManager::drawImage(const QPixmap &dcSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
                                      QRect rcSizingMargins, bool alphaBlend) const
{
    baseStyle()->drawImage(dcSrc, p, rcDest, rcSrc, rcSizingMargins, alphaBlend);
}

void QdfCommonPaintManager::drawPixmap(const QPixmap &dcSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
...
void QdfCommonStylePrivate::doTimer()
{
    for (int i = m_animations.size() - 1; i >= 0; --i)
    {
        ...
        if (!pWidget || !pWidget->isEnabled() || !pWidget->isVisible() || pWidget->window()->isMinimized() ||
            !pItem->running())
        {
...
void QdfCommonStylePrivate::setPalette(QWidget *widget) { m_customPaletteWidgets.insert(widget, widget->palette()); }
...
QdfCommonStyle::~QdfCommonStyle() { QDF_FINI_PRIVATE(); }

QColor QdfCommonStyle::getThemeColor(const QString &sectionName, const QString &keyName, QColor color) const
{
...
void QdfCommonStyle::drawControl(ControlElement element, const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    bool ret = false;
    bool qtcStyle = false;
    switch (element)
...
QSize QdfCommonStyle::sizeFromContents(ContentsType ct, const QStyleOption *opt, const QSize &contentsSize,
                              ...
                              QStyleHintReturn *returnData) const
{
    ...
    {
        if (QdfStyleHintReturnThemeColor *vret = qstyleoption_cast<QdfStyleHintReturnThemeColor *>(returnData))
        {
...
QPixmap QdfCommonStyle::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt, const QWidget *widget) const
{
    QDF_D(const QdfCommonStyle);
    if (d->m_defaultStyle)
    {
...
QPixmap QdfCommonStyle::cached(const QString &img) const { return ::cachedPixmap(img); }

void QdfCommonStyle::clearCache() { QPixmapCache::clear(); }

void QdfCommonStyle::drawImagePart(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
                                   bool alpha) const
{
    Q_UNUSED(alpha);
...
void QdfCommonStyle::drawImage(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
                               QRect rcSizingMargins, QColor clrTransparent, bool alphaBlend) const
{
...
void QdfCommonStyle::drawImage(const QPixmap &dcSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
                               QRect rcSizingMargins, bool alphaBlend) const
{
...
void QdfCommonStyle::drawPixmap(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc, bool alpha,
                                QRect rcSizingMargins, bool alphaBlend) const
{
    Q_UNUSED(alphaBlend)
    QPixmap dcSrc = soSrc;
    ...
    */
    ...
    else
    {
        ...
        drawImagePart(
                dcSrc, p,
                qdf_Rect(rc.left(), rc.top(), rc.left() + rcDestSizingMargins.left(), rc.top() + rcDestSizingMargins.top()),
        ...
        // left-center
        drawImagePart(dcSrc, p,
                      qdf_Rect(rc.left(), rc.top() + rcDestSizingMargins.top(), rc.left() + rcDestSizingMargins.left(),
        ...
        // right-center
        drawImagePart(dcSrc, p,
                      qdf_Rect(rc.right() - rcDestSizingMargins.right(), rc.top() + rcDestSizingMargins.top(),
                               rc.right(), rc.bottom() - rcDestSizingMargins.bottom()),
                      qdf_Rect(rcImage.right() - rcSizingMargins.right(), rcImage.top() + rcSizingMargins.top(),
        ...
        // bottom-center
        drawImagePart(dcSrc, p,
                      qdf_Rect(rc.left() + rcDestSizingMargins.left(), rc.bottom() - rcDestSizingMargins.bottom(),
                               rc.right() - rcDestSizingMargins.right(), rc.bottom()),
                      qdf_Rect(rcImage.left() + rcSizingMargins.left(), rcImage.bottom() - rcSizingMargins.bottom(),
                               rcImage.right() - rcSizingMargins.right(), rcImage.bottom()),
                      alpha);
        // right-bottom
        drawImagePart(dcSrc, p,
                      qdf_Rect(rc.right() - rcDestSizingMargins.right(), rc.bottom() - rcDestSizingMargins.bottom(),
                               rc.right(), rc.bottom()),
                      qdf_Rect(rcImage.right() - rcSizingMargins.right(), rcImage.bottom() - rcSizingMargins.bottom(),
...
bool QdfCommonStyle::transitionsEnabled() const { return false; }
                                    ...
                                    const QWidget *widget, int iDurationOver) const
{
    ...
    if (widget && !wasDetermined && bAnimation)
    {
        ...
        bool doTransition =
                ((state & State_Sunken) != (oldState & State_Sunken) || (state & State_On) != (oldState & State_On) ||
