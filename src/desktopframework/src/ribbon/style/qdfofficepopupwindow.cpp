...
void QdfManagerPopup::appendPopup(QdfOfficePopupWindow *popup) { m_listPopup.append(popup); }

void QdfManagerPopup::removePopup(QdfOfficePopupWindow *popup) { m_listPopup.removeOne(popup); }
        ...
        while (index < m_listPopup.count())
        {
            ...
            if (curPopup)
            {
                ...
                if (rcPopup.intersects(rcActivePopup))
                {
                    ...
                    rcActivePopup =
                            QRect(QPoint(ptPopup.x() - szPopup.width(), ptPopup.y() - szPopup.height()), szPopup);
                    intersect = true;
                }
            }
            index++;
...
void QdfOfficePopupWindowPrivate::showPopup()
{
    ...
    m_stateCurrent.rcPopup =
            QRect(QPoint(positionPopup.x() - sizePopup.width(), positionPopup.y() - sizePopup.height()), sizePopup);
...
void QdfOfficePopupWindowPrivate::handleMousePressEvent(QMouseEvent *event)
{
    ...
    {
        QStyle::SubControl ctrl = q->style()->hitTestComplexControl(QStyle::CC_TitleBar, &optTitle, event->pos(), q);
        m_buttonDown = ctrl;
        m_pressed = true;
...
void QdfOfficePopupWindowPrivate::initFormStyleOption(QStyleOptionFrame *option) const
{
    ...
    {
        QRect buttonRect =
                q->style()->subControlRect(QStyle::CC_TitleBar, &optTitle, QStyle::SC_TitleBarCloseButton, q);
        szTitle.setHeight(buttonRect.height());
    }

    QRect labelRect = QRect(
            QPoint(0, 0), QSize(szTitle.width(), titleBarHeight > 7 ? titleBarHeight : titleBarHeight + szTitle.height()));
...
PopupState QdfOfficePopupWindowPrivate::getPopupState() const { return m_popupState; }
...
void QdfOfficePopupWindowPrivate::calclayout()
{
    ...
    QSize szTitle = m_title->sizeHint();
    QRect buttonRect = q->style()->subControlRect(QStyle::CC_TitleBar, &optTitle, QStyle::SC_TitleBarCloseButton, q);
