...
void QdfRibbonGroupPrivate::initStyleOption(QStyleOptionGroupBox &opt) const
{
    ...
    opt.lineWidth = q->style()->pixelMetric((QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonHeightCaptionGroup, &opt, q);
...
int QdfRibbonGroupPrivate::calcMinimumWidth(QdfRibbonControlSizeDefinition::GroupSize size) const
{
    ...
    int width = q->style()->pixelMetric((QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonReducedGroupWidth, &opt, q);
...
int QdfRibbonGroupPrivate::arrangeRowContent(QList<QdfRibbonControl *> &row, int leftOffset, int rowItemHeight,
...
void QdfRibbonGroupPrivate::updateOptionButtonLayout()
{
    ...
    if (m_optionButton && m_butOptionVisible)
    {
        QStyleOption opt;
        opt.init(q);
        int width = q->style()->pixelMetric((QStyle::PixelMetric) QdfRibbonStyle::PM_RibbonHeightCaptionGroup, &opt, q);
...
QSize QdfRibbonGroupPrivate::updateControlsLayout(QWidget *parent, int leftOffset) const
{
    ...
    for (QList<QdfRibbonControl *>::const_iterator it = controls.constBegin(); it != controls.constEnd(); ++it)
    {
        QdfRibbonControl *ribbonControl = *it;
        QdfRibbonColumnBreakControl *sepatator = qobject_cast<QdfRibbonColumnBreakControl *>(ribbonControl);
...
{
    for (QList<QdfRibbonControl *>::const_iterator it = m_controls.constBegin(); it != m_controls.constEnd(); ++it)
    {
...
bool QdfRibbonGroupPrivate::canReduce() { return m_currentSize < QdfRibbonControlSizeDefinition::GroupPopup; }

bool QdfRibbonGroupPrivate::canExpand() { return m_currentSize > QdfRibbonControlSizeDefinition::GroupLarge; }
...
bool QdfRibbonGroup::isReduced() const
{
    ...
    {
        if (!d->m_controls.at(0)->sizeDefinition(QdfRibbonControlSizeDefinition::GroupLarge)->isStretchable())
        {
            return false;
        }
    }
...
QdfRibbonControl *QdfRibbonGroup::controlByAction(QAction *action) const
{
    ...
    for (QList<QdfRibbonControl *>::const_iterator it = d->m_controls.constBegin(); it != d->m_controls.constEnd(); ++it)
    {
...
QAction *QdfRibbonGroup::addAction(const QIcon &icon, const QString &text, Qt::ToolButtonStyle style, QMenu *menu,
...
QAction *QdfRibbonGroup::insertAction(QAction *before, QAction *action, Qt::ToolButtonStyle style, QMenu *menu,
                                      QToolButton::ToolButtonPopupMode mode)
{
    QDF_D(QdfRibbonGroup);
...
QMenu *QdfRibbonGroup::addMenu(const QIcon &icon, const QString &text, Qt::ToolButtonStyle style)
{
    ...
    {
        QdfRibbonButtonAction *action = new QdfRibbonButtonAction(this, icon, text, style, menu, QToolButton::InstantPopup);
        addAction(action);
    }
    return menu;
}
                ...
                if (control == nullptr)
                {
                    if (QWidgetAction *widgetAction = qobject_cast<QWidgetAction *>(action))
                    {
                        ...
                        if (widget != nullptr)
                        {
                            ...
                            if (control == nullptr)
                            {
                                QdfRibbonWidgetControl *widgetControl = new QdfRibbonWidgetControl(this, false);
