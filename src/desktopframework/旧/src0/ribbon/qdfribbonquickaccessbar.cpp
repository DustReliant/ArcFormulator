...
void QdfRibbonQuickAccessButton::paintEvent(QPaintEvent *event)
{
    ...
    initStyleOption(&opt);
    style()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonQuickAccessButton, &opt, &p, this);
}
...
void QdfRibbonQuickAccessBarPrivate::init()
{
    ...
    m_actionAccessPopup->setProperty(__qdf_Quick_Access_Button, QLatin1String("__qtc_Quick_Access_Button"));
...
QdfRibbonQuickAccessBar::~QdfRibbonQuickAccessBar() { QDF_FINI_PRIVATE(); }
...
void QdfRibbonQuickAccessBar::aboutToShowCustomizeMenu()
{
    ...
    d->m_menu->addSeparator()->setText(QdfRibbonBar::tr_compatible(RibbonCustomizeQuickAccessToolBarString));
