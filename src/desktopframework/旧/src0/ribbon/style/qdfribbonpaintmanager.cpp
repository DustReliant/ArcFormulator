...
QdfRibbonPaintManager::QdfRibbonPaintManager(QdfCommonStyle *baseStyle) : QdfOfficePaintManager(baseStyle) {}

QdfRibbonPaintManager::~QdfRibbonPaintManager() {}

QIcon QdfRibbonPaintManager::standardIconEx(QStyle::StandardPixmap px, const QStyleOption *option, const QWidget *wd,
                                            bool &ret) const
{
    Q_UNUSED(px);
    Q_UNUSED(option);
...
void QdfRibbonPaintManager::drawRibbonBar(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget)
    if (const QdfStyleOptionRibbon *optRibbonBar = qstyleoption_cast<const QdfStyleOptionRibbon *>(option))
    {
...
void QdfRibbonPaintManager::drawPixTitleBackground(QPainter *painter, const QRect &rectDst, const QPixmap &pixTitleBackground,
...
void QdfRibbonPaintManager::drawRibbonTabBar(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
...
void QdfRibbonPaintManager::drawRibbonGroups(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    QDF_D_STYLE(QdfRibbonStyle)
...
void QdfRibbonPaintManager::drawGroup(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget)
    QDF_D_STYLE(QdfRibbonStyle)
...
void QdfRibbonPaintManager::drawReducedGroup(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(QdfRibbonStyle)
    if (const QStyleOptionToolButton *optGroup = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
        ...
        QPen savePen = painter->pen();
        painter->setPen(d->m_clrRibbonSeparator);
        painter->drawLine(QPoint(rcEntry.right() - 1, rcEntry.top()), QPoint(rcEntry.right() - 1, rcEntry.bottom()));
        painter->setPen(savePen);
        ...
        if (!strFirstRow.isEmpty())
        {
            ...
            baseStyle()->proxy()->drawItemText(painter, rectFirstRow, flags | Qt::AlignHCenter, optGroup->palette,
        ...
        if (!strSecondRow.isEmpty())
        {
            ...
            baseStyle()->proxy()->drawItemText(painter, rectSecondRow, flags | Qt::AlignLeft, optGroup->palette,
...
bool QdfRibbonPaintManager::drawToolBar(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
        ...
        if (const QStyleOptionGroupBox *optGroup = qstyleoption_cast<const QStyleOptionGroupBox *>(option))
        {
            ...
            else
            {
                ...
                if (optGroup->textAlignment == Qt::AlignBottom)
                {
                    ...
                    baseStyle()->proxy()->drawItemText(painter, rect.adjusted(indent + 1, 1, -indent - 1, -1),
                ...
                else if (optGroup->textAlignment == Qt::AlignTop)
                {
                    ...
                    baseStyle()->proxy()->drawItemText(
                            painter, rect.adjusted(indent + rect.width() - rectText.width() - 2, 1, -indent - 1, -1),
                            Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal,
                ...
                else if (optGroup->textAlignment == Qt::AlignVCenter)
                {
                    painter->save();
                    painter->setPen(d->m_clrRibbonSeparator);
                    painter->drawLine(rect.center().x(), rect.top() + rectText.width() + rectText.width() + 4,
                    ...
                    pal.setColor(QPalette::WindowText, d->m_clrRibbonSeparator.darker());
                    baseStyle()->proxy()->drawItemText(
                            painter, rect.adjusted(indent + (rect.width() - rectText.width()) / 2 - 2, 1, -indent - 1, -1),
                            Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal,
                            optGroup->state & QStyle::State_Enabled, titleText, QPalette::WindowText);
                    painter->restore();
                }
...
bool QdfRibbonPaintManager::drawPanelButtonTool(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    if (!widget)
    {
        return false;
    ...
    if (widget && !qobject_cast<const QdfRibbonTitleButton *>(widget))
    {
        if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
        {
            ...
            QdfRibbonToolBarControl *toolBarControl = qobject_cast<QdfRibbonToolBarControl *>(widget->parentWidget());
            if (toolBarControl)
            {
                smallSize = true;
            }
            ...
            bool mouseInSplit =
                    option->state & QStyle::State_MouseOver && toolbutton->activeSubControls & QStyle::SC_ToolButton;
            bool mouseInSplitDropDown =
                    option->state & QStyle::State_MouseOver && toolbutton->activeSubControls & QStyle::SC_ToolButtonMenu;
            bool pressed = option->state & QStyle::State_Sunken;
            bool popuped =
                    (toolbutton->activeSubControls & QStyle::SC_ToolButtonMenu) && (option->state & QStyle::State_Sunken);
            ...
            if (!(toolbutton->features & QStyleOptionToolButton::MenuButtonPopup))
            {
                ...
                if (!autoRaise && !qobject_cast<const QStatusBar *>(widget->parentWidget()))
                {
                    ...
                    baseStyle()->proxy()->drawPrimitive(QStyle::PE_PanelButtonCommand, &tmpBtn, painter, widget);
                }
                else
                {
                    QColor color;
            ...
            if (mouseInSplit || checked)
            {
                ...
                else if (checked)
                {
                    ...
                    {
                        colorSplit = !mouseInSplit ? buttonHiliteColor(widget, true /*highlightDn*/) : QColor();
                    }
                }
                else
                {
    ...
    {
        if (const QdfStyleCaptionButton *toolbutton = qstyleoption_cast<const QdfStyleCaptionButton *>(option))
        {
            ...
            QColor colText = titleButtonHiliteColor(!(toolbutton->state & QStyle::State_Enabled), widget);
...
{
    if (const QdfStyleOptionTitleBar *optTitleBar = qstyleoption_cast<const QdfStyleOptionTitleBar *>(option))
    {
...
bool QdfRibbonPaintManager::drawSlider(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (const QStyleOptionSlider *slider = qstyleoption_cast<const QStyleOptionSlider *>(option))
    {
        ...
        {
            QRect groove = baseStyle()->proxy()->subControlRect(QStyle::CC_Slider, option, QStyle::SC_SliderGroove, widget);
            QRect handle = baseStyle()->proxy()->subControlRect(QStyle::CC_Slider, option, QStyle::SC_SliderHandle, widget);
            ...
            if ((option->subControls & QStyle::SC_SliderHandle))
            {
                bool enabled = option->state & QStyle::State_Enabled;
                bool selected =
                        slider->activeSubControls & QStyle::SC_SliderHandle && option->state & QStyle::State_MouseOver;
...
void QdfRibbonPaintManager::drawGroupScrollButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget)
    if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
        ...
        ImageIcons iImage = toolbutton->arrowType == Qt::LeftArrow ? QdfOfficePaintManager::Icon_ArowLeft
...
bool QdfRibbonPaintManager::drawFrameMenu(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(const QdfRibbonStyle)
    if (qobject_cast<const QdfRibbonSystemPopupBar *>(widget) || qobject_cast<const QdfRibbonPageSystemPopup *>(widget))
    {
        QRect rect = option->rect;
...
void QdfRibbonPaintManager::drawTabShape(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(QdfRibbonStyle)
    if (const QdfStyleRibbonOptionHeader *optTab = qstyleoption_cast<const QdfStyleRibbonOptionHeader *>(option))
    {
        if (optTab->state & QStyle::State_Selected)
        {
            QRect rectTab = optTab->rect;
            painter->fillRect(rectTab, d->m_isActiveTabAccented
...
void QdfRibbonPaintManager::drawTabShapeLabel(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(QdfRibbonStyle)
    if (const QStyleOptionHeader *optTab = qstyleoption_cast<const QStyleOptionHeader *>(option))
    {
        ...
        baseStyle()->proxy()->drawItemText(painter, optTab->rect, flags, pal, option->state & QStyle::State_Enabled, text,
                                           QPalette::WindowText);
    }
}

bool QdfRibbonPaintManager::drawTabBarTabShape(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    return QdfOfficePaintManager::drawTabBarTabShape(option, painter, widget);
}

bool QdfRibbonPaintManager::drawShapedFrame(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    return QdfOfficePaintManager::drawShapedFrame(option, painter, widget);
}

void QdfRibbonPaintManager::drawSystemButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(const QdfRibbonStyle)
    if (const QdfStyleSystemToolButton *toolbutton = qstyleoption_cast<const QdfStyleSystemToolButton *>(option))
    {
        if (qobject_cast<const QdfRibbonBar *>(widget->parentWidget()))
        {
            if (toolbutton->toolButtonStyle != Qt::ToolButtonFollowStyle)
            {
                ...
                {
                    painter->fillRect(toolbutton->rect, isHighlighted ? m_clrAccentHilight : d->m_clrAccent);
                }
        ...
        painter->fillRect(toolbutton->rect.adjusted(0, 0, -1, -1), isHighlighted ? m_clrAccentHilight : d->m_clrAccent);
...
void QdfRibbonPaintManager::drawQuickAccessButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
...
void QdfRibbonPaintManager::drawOptionButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
...
void QdfRibbonPaintManager::drawPopupResizeGripper(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    ...
    if (const QStyleOptionSizeGrip *sizeGrip = qstyleoption_cast<const QStyleOptionSizeGrip *>(option))
    {
        ...
        for (int y = 0; y < 3; y++)
        {
            for (int x = 0; x < 3 - y; x++)
            {
                painter->fillRect(QRect(QPoint(pt.x() + 1 - x * 4, pt.y() + 1 - y * 4), QSize(2, 2)), QColor(255, 255, 255));
...
bool QdfRibbonPaintManager::drawPanelStatusBar(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    return QdfOfficePaintManager::drawPanelStatusBar(option, painter, widget);
}

bool QdfRibbonPaintManager::drawSizeGrip(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    return QdfOfficePaintManager::drawSizeGrip(option, painter, widget);
}

bool QdfRibbonPaintManager::drawIndicatorArrow(QStyle::PrimitiveElement pe, const QStyleOption *option, QPainter *painter,
                                               const QWidget *widget) const
{
...
void QdfRibbonPaintManager::drawRectangle(QPainter *painter, const QRect &rect, bool selected, bool pressed, bool enabled,
                                          bool checked, bool popuped, Qdf::BarType barType,
                                          Qdf::BarPosition barPos) const
{
    QdfOfficePaintManager::drawRectangle(painter, rect, selected, pressed, enabled, checked, popuped, barType, barPos);
}

void QdfRibbonPaintManager::drawKeyTip(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    QDF_D_STYLE(const QdfRibbonStyle)
    painter->fillRect(option->rect, m_clrBarHilite);
    QdfDrawHelpers::draw3dRectEx(*painter, option->rect, d->m_clrRibbonSeparator, d->m_clrRibbonSeparator);
}

void QdfRibbonPaintManager::drawBackstage(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    QDF_D_STYLE(const QdfRibbonStyle)
    if (const QdfStyleOptionRibbonBackstage *optBackstage = qstyleoption_cast<const QdfStyleOptionRibbonBackstage *>(option))
    {
        painter->fillRect(option->rect, m_clrBarLight);
        int top =
                (bool) baseStyle()->proxy()->styleHint((QStyle::StyleHint) QdfRibbonStyle::SH_RibbonBackstageHideTabs) ? 0 : 2;
        QRect rc = QRect(QPoint(0, top), QPoint(optBackstage->menuWidth, option->rect.bottom()));
        painter->fillRect(rc, d->m_clrAccent);
    }
}

void QdfRibbonPaintManager::drawRibbonBackstageMenu(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    QDF_D_STYLE(const QdfRibbonStyle)
    if (const QStyleOptionMenuItem *menuitem = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
    {
        ...
        if (menuitem->menuItemType == QStyleOptionMenuItem::Separator)
        {
            ...
            const int iconSize = baseStyle()->proxy()->pixelMetric(QStyle::PM_ToolBarIconSize, option, widget);
            int yoff = (y - 1 + h / 2);
        ...
        if (!menuitem->text.isEmpty())
        {
            ...
            // draw text
            baseStyle()->proxy()->drawItemText(painter, rcText, flags, pal, option->state & QStyle::State_Enabled,
                                               menuitem->text, QPalette::WindowText);
        }
    }
}

void QdfRibbonPaintManager::drawRibbonBackstageCloseButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);

    painter->fillRect(option->rect, backstageCloseButton());

    if (const QStyleOptionToolButton *optButton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
...
bool QdfRibbonPaintManager::drawRibbonBackstageSeparator(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    bool ret = false;
...
#endif
    {
        ...
        {
            QRect rc(QPoint(option->rect.left(), option->rect.height() / 2), QSize(option->rect.width(), 1));
            painter->fillRect(rc, QColor(170, 170, 170));
            ret = true;
        }
        else if (f->frameShape == QFrame::VLine)
        {
            QRect rc(QPoint(option->rect.width() / 2, option->rect.top()), QSize(1, option->rect.height()));
            painter->fillRect(rc, QColor(170, 170, 170));
            ret = true;
        }
    }
...
void QdfRibbonPaintManager::drawRibbonSliderButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    QDF_D_STYLE(const QdfRibbonStyle)
    ...
    {
        clrLine = widget && getParentWidget<const QStatusBar>(widget) ? QColor(255, 255, 255) : m_clrBarText;
    }
...
void QdfRibbonPaintManager::fillRibbonButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    bool popuped = false;
    if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
...
void QdfRibbonPaintManager::fillRibbonTabControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    if (const QdfRibbonTab *tab = qobject_cast<const QdfRibbonTab *>(widget))
    {
...
void QdfRibbonPaintManager::drawPanelBackstageButton(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(widget);
    QDF_D_STYLE(QdfRibbonStyle)
    if (const QdfStyleOptionBackstageButton *btnStyle = qstyleoption_cast<const QdfStyleOptionBackstageButton *>(option))
    {
        ...
        bool highlighted = option->state & QStyle::State_MouseOver;
        bool pressed = option->state & QStyle::State_Sunken;
        bool popuped = (btnStyle->activeSubControls & QStyle::SC_ToolButtonMenu) && (option->state & QStyle::State_Sunken);
