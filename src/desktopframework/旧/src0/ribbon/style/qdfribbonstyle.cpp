...
void QdfRibbonStylePrivate::tabLayout(const QStyleOptionTabV3 *opt, const QWidget *widget, QRect *textRect,
...
{
    QDF_Q(const QdfRibbonStyle);
    QdfRibbonPaintManagerInterface *ribbonPaintManager = qobject_cast<QdfRibbonPaintManagerInterface *>(q->paintManager());
...
QdfRibbonStyle::QdfRibbonStyle(QMainWindow *mainWindow) : QdfOfficeStyle(mainWindow, new QdfRibbonStylePrivate)
{
    QDF_D(QdfRibbonStyle);
    d->initialization();
}
...
void QdfRibbonStyle::polish(QWidget *widget)
{
    ...
    else if ((qobject_cast<QdfRibbonGroup *>(widget->parentWidget()) && qobject_cast<QToolButton *>(widget)))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Light, d->m_clrToolBarGrayText);
        palette.setColor(QPalette::WindowText, d->m_clrToolBarText);
    ...
    else if (qobject_cast<QdfRibbonSystemPopupBar *>(widget->parentWidget()) && qobject_cast<QToolButton *>(widget))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Light, d->m_clrMenuBarGrayText);
    ...
    else if (qobject_cast<QdfOfficePopupMenu *>(widget->parentWidget()) && qobject_cast<QLabel *>(widget))
    {
        QPalette palette = widget->palette();
...
void QdfRibbonStyle::drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter,
...
void QdfRibbonStyle::drawControl(ControlElement element, const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    switch (element)
    {
...
void QdfRibbonStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *option, QPainter *painter,
                                        const QWidget *widget) const
{
...
int QdfRibbonStyle::pixelMetric(PixelMetric pm, const QStyleOption *option, const QWidget *widget) const
{
    ...
    switch (pm)
    {
        ...
        case PM_SmallIconSize:
            {
                if (widget && (qobject_cast<const QdfRibbonStatusBar *>(widget->parentWidget()) ||
        ...
        case PM_RibbonHeightCaptionGroup:
            ...
            {
                bool titleGroupsVisible = widget ? widget->property(_qdf_TitleGroupsVisible).toBool() : false;
...
QPixmap QdfRibbonStyle::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt, const QWidget *widget) const
{
        ...
        case CT_ToolButton:
            {
                ...
                else if (widget && (qobject_cast<const QdfRibbonStatusBar *>(widget->parentWidget()) ||
                                    qobject_cast<const QdfRibbonStatusBarSwitchGroup *>(widget->parentWidget())))
                {
...
void QdfRibbonStyle::drawRibbonGroups(const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
bool QdfRibbonStyle::drawIndicatorToolBarSeparator(const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
bool QdfRibbonStyle::drawTitleBar(const QStyleOptionComplex *opt, QPainter *p, const QWidget *w) const
{
    ...
    if (const QdfStyleOptionTitleBar *optTitleBar = qstyleoption_cast<const QdfStyleOptionTitleBar *>(opt))
    {
        if (optTitleBar->frameHelper)
        {
            ...
            {
                proxy()->drawPrimitive((QStyle::PrimitiveElement) QdfRibbonStyle::PE_RibbonContextHeaders, opt, p, w);
            }
                    ...
                    optTitleBar->titleBarFlags & Qt::WindowSystemMenuHint)
                {
                    ...
                    if (!optTitleBar->icon.isNull())
                    {
                        ...
                        if (HICON hIcon = optTitleBar->frameHelper->windowIcon(optTitleBar->icon, optTitleBar->rect))
                        {
                            ...
                            ::DrawIconEx(p->paintEngine()->getDC(), ir.left(), ir.top(), hIcon, 0, 0, 0, nullptr,
                                         DI_NORMAL | DI_COMPAT);
    #endif
                        }
#else
...
void QdfRibbonStyle::drawSystemButton(const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
...
void QdfRibbonStyle::drawOptionButton(const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
void QdfRibbonStyle::drawGroupScrollButton(const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
void QdfRibbonStyle::drawFileButtonLabel(const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
void QdfRibbonStyle::drawPopupSystemButton(const QStyleOption *option, QPainter *p, const QWidget *w) const
{
    Q_UNUSED(option);
    Q_UNUSED(p);
    Q_UNUSED(w);
...
bool QdfRibbonStyle::drawTabBarTabShape(const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
bool QdfRibbonStyle::drawTabBarTabLabel(const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    QDF_D(const QdfRibbonStyle);
...
void QdfRibbonStyle::calcRects(const QRect &rc, const QSize &pmSize, const QFontMetrics &fMetric, const QString &text,
...
bool QdfRibbonStyle::drawToolButtonLabel(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    ...
    {
        if (const QdfStyleOptionBackstageButton *toolbutton = qstyleoption_cast<const QdfStyleOptionBackstageButton *>(opt))
        {
...
bool QdfRibbonStyle::drawGroupControlEntry(const QStyleOption *opt, QPainter *p, const QWidget *widget) const
{
    Q_UNUSED(opt);
    Q_UNUSED(p);
    Q_UNUSED(widget);
...
bool QdfRibbonStyle::drawIndicatorArrow(PrimitiveElement pe, const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    QDF_D(const QdfRibbonStyle);
...
void QdfRibbonStyle::drawRectangle(QPainter *p, const QRect &rect, bool selected, bool pressed, bool enabled, bool checked,
...
void QdfRibbonStyle::drawSplitButtonPopup(QPainter *p, const QRect &rect, bool selected, bool enabled, bool popuped) const
{
    Q_UNUSED(p);
    Q_UNUSED(rect);
    Q_UNUSED(selected);
...
void QdfRibbonStyle::drawItemText(QPainter *painter, const QRect &rect, int alignment, const QPalette &pal, bool enabled,
...
void QdfRibbonStyle::drawRibbonBackstageCloseButton(const QStyleOption *opt, QPainter *p, const QWidget *w) const
{
    QDF_D(const QdfRibbonStyle);
...
bool QdfRibbonStyle::showToolTip(const QPoint &pos, QWidget *w)
{
    ...
    if (getParentWidget<QdfRibbonBar>(w))
    {
        ...
        if (const QdfRibbonGroup *group = getParentWidget<QdfRibbonGroup>(w))
        {
            ...
            {
                posY = group->mapToGlobal(group->rect().bottomRight()).y() + int(QdfDrawHelpers::dpiScaled(2.));
            }
            else if (const QdfRibbonPage *page = getParentWidget<QdfRibbonPage>(w))
            {
                posY = page->mapToGlobal(page->rect().bottomRight()).y() + int(QdfDrawHelpers::dpiScaled(2.));
            }
        }
        ...
        else if (strTipText.isEmpty())
        {
            ...
            if (!qobject_cast<QdfRibbonGroup *>(widget))
            {
                if (QdfRibbonPageSystemRecentFileList *recentFile = qobject_cast<QdfRibbonPageSystemRecentFileList *>(w))
                {
    ...
    switch (event->type())
    {
        ...
        case QEvent::Show:
            if (QMenu *menu = qobject_cast<QMenu *>(watched))
            {
                ...
                {
                    if (ribbonBar->qdf_d()->m_levels.size() > 0 || ((qobject_cast<QdfRibbonPageSystemPopup *>(watched) ||
...
QStringList QdfRibbonStylePlugin::keys() const { return QStringList() << "QdfRibbonStyle"; }
