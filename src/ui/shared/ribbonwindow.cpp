...
RibbonWindow::RibbonWindow(QWidget *parent)
    : QdfMainWindow(parent), m_ribbonStyle(qobject_cast<Qdf::QdfRibbonStyle *>(qApp->style()))
{
    // m_ribbonStyle->setDPIAware(false);
    QdfRibbonToolTip::setWrapMode(QdfRibbonToolTip::NativeWrap);
    createOptions();
...
{
    Qdf::QdfRibbonStyle::Theme themeId = Qdf::QdfRibbonStyle::OfficeWhite;
    if (m_ribbonStyle)
...
void RibbonWindow::optionsTheme(QAction *action)
{
    ...
    if (action->objectName() == "OfficeWhite")
        themeId = QdfOfficeStyle::OfficeWhite;
    else if (action->objectName() == "OfficeGray")
        themeId = QdfOfficeStyle::OfficeGray;
    else if (action->objectName() == "OfficeDark")
        themeId = QdfOfficeStyle::OfficeDark;

    setStyleTheme(themeId);
}

QdfRibbonStyle::Theme RibbonWindow::styleTheme() const
{
        if (m_ribbonStyle == nullptr) {
            Q_ASSERT(false);
            return (QdfRibbonStyle::Theme) -1;
        }
    return m_ribbonStyle->getTheme();
}

void RibbonWindow::setStyleTheme(QdfRibbonStyle::Theme themeId)
{
        if (m_ribbonStyle == nullptr) {
            Q_ASSERT(false);
            return;
...
void RibbonWindow::setButtonStyleTheme(QdfRibbonStyle::Theme themeId)
{
        if (QToolButton *button = ribbonBar()->getSystemButton()) {
            if (themeId == QdfOfficeStyle::OfficeWhite || themeId == QdfOfficeStyle::OfficeGray ||
                themeId == QdfOfficeStyle::OfficeDark)
...
{
    QdfOfficeStyle::Theme theme = m_ribbonStyle->getTheme();

    QString strTheme = "OfficeWhite";
    if (theme == QdfOfficeStyle::OfficeWhite)
        strTheme = "OfficeWhite";
    else if (theme == QdfOfficeStyle::OfficeGray)
        strTheme = "OfficeGray";
    else if (theme == QdfOfficeStyle::OfficeDark)
        strTheme = "OfficeDark";
...
{
    if (Qdf::QdfOfficeStyle *st = qobject_cast<Qdf::QdfOfficeStyle *>(qApp->style()))
...
void RibbonWindow::showQuickAccessCustomizeMenu(QMenu *menu)
{
    ...
    menu->addAction(ribbonBar()->quickAccessBarPosition() == QdfRibbonBar::QATopPosition ? m_actionShowBelowRibbon
...
{
    ribbonBar()->setQuickAccessBarPosition(ribbonBar()->quickAccessBarPosition() == QdfRibbonBar::QATopPosition
